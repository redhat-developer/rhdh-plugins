...
[[ "${GITHUB_ISSUE_URL}" =~ ^https://github\.com/[A-Za-z0-9._-]+/[A-Za-z0-9._-]+/pull/[1-9][0-9]*$ ]] || {
  echo "::error::CI fix requires a GitHub pull request URL" >&2
  exit 1
}
[[ -f "${CI_CONTEXT_FILE}" ]] || { echo "::error::Trusted dispatch context is missing" >&2; exit 1; }
jq -e '
  ._fullsend_ci.version == 1
  and ._fullsend_ci.kind == "fix"
  and ._fullsend_ci.automation_mode == "repair"
  and (._fullsend_ci.run_id | type == "number")
  and (._fullsend_ci.run_attempt | type == "number")
  and (._fullsend_ci.head_sha | test("^[0-9a-f]{40}$"))
  and (._fullsend_ci.workspace | test("^[a-z0-9][a-z0-9-]*$"))
  and (._fullsend_ci.iteration == 1 or ._fullsend_ci.iteration == 2)
...
[[ "$(jq -r '.head.repo.full_name' <<<"${current_pr}")" == "${REPO_FULL_NAME}" ]] || { echo "::error::Fork PRs are diagnosis-only" >&2; exit 1; }
if jq -e '.labels | any(.[]; .name == "fullsend-no-fix")' <<<"${current_pr}" >/dev/null; then
  echo "::error::fullsend-no-fix is active" >&2
  exit 1
fi

repo_dir="${TARGET_REPO:-${GITHUB_WORKSPACE:-}/target-repo}"
head_ref="$(jq -r '.head.ref' <<<"${current_pr}")"
[[ -d "${repo_dir}/.git" ]] || { echo "::error::Target repository checkout is missing" >&2; exit 1; }
git -C "${repo_dir}" config core.hooksPath /dev/null
gh auth setup-git
git -C "${repo_dir}" fetch --no-tags --depth=1 origin "refs/heads/${head_ref}"
git -C "${repo_dir}" checkout -B "${head_ref}" FETCH_HEAD
[[ "$(git -C "${repo_dir}" rev-parse HEAD)" == "${head_sha}" ]] || {
  echo "::error::Fetched branch does not match the analyzed head" >&2
  exit 1
}
...
echo "CI fix preflight passed for PR #${pr_number} at ${head_sha:0:12}"
