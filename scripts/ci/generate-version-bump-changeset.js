...
async function main() {
  ...
  // Get the packages for this workspace filtering down to just those in the `@red-hat-developer-hub` org
  ...
  const packageEntries = packages
    .filter(p => p.packageJson.name.includes('@red-hat-developer-hub'))
