...
export async function createNotebooksRouter(
  ...
  const lightSpeedPort =
    ...
    DEFAULT_LIGHTSPEED_SERVICE_PORT;
  const lcsBaseUrl = `http://${DEFAULT_LIGHTSPEED_SERVICE_HOST}:${lightSpeedPort}`;
  ...
  logger.info(`AI Notebooks connecting to Lightspeed-Core at ${lcsBaseUrl}`);
    ...
    withAuth(async (req, res, userId) => {
      ...
      for (let retries = 0; retries <= MAX_QUERY_RETRIES; retries++) {
        ...
        const response = await fetch(
          `${lcsBaseUrl}/v1/responses?user_id=${encodeURIComponent(userId)}`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(lcsRequest),
        ...
        if (
          ...
          delete lcsRequest.conversation;
          conversationId = null;
          continue;
        }
