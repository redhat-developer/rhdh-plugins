 ...
 *
 * Reads connection settings from `boost.providers.ogx` in app-config.yaml
 * and initializes:
...
export class ResponsesApiProviderFactory {
  ...
  create(): ResponsesApiProviderBundle {
    ...
    this.logger.info(
      `Creating OGX provider for endpoint: ${connection.baseUrl}`,
    );
  ...
  /**
   * Read OGX connection settings from app-config.yaml.
   *
   ...
   * ```
   */
  private readConnectionConfig(): OgxConnectionConfig {
    const providerConfig = this.config.getOptionalConfig('boost.providers.ogx');

    if (!providerConfig) {
      this.logger.warn(
        'No boost.providers.ogx config found. ' +
