...
/**
 * Session data tracked by the Kagenti provider.
 *
 * @internal
 */
export interface SessionData {
  /** The A2A session ID for multi-turn tracking. */
  sessionId: string;
...
/**
 * Maps conversation IDs to Kagenti A2A session data using Backstage cacheService.
 *
 * Enables multi-turn conversations by tracking the A2A session ID chain.
 * Uses cacheService with a 24-hour session TTL (task 3.7).
 * Redis-backed in production for multi-instance safety.
 *
 * @internal
 */
export class SessionMap {
  private static readonly KEY_PREFIX = 'kagenti:session:';
  ...
  async set(conversationId: string, data: SessionData): Promise<void> {
    ...
    this.logger.debug(
      `Updated session for conversation "${conversationId}" (agent: ${data.agentId})`,
    );
  }
