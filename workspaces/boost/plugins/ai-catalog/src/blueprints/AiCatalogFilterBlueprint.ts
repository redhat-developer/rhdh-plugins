...
export interface FilterDefinition {
  ...
  label: string;
  /** Translation key resolved via aiCatalogTranslationRef. Takes precedence over `label`. */
  labelKey?: string;
...
const RESERVED_PARAMS = ['q', 'view', 'page', 'pageSize'];
...
export const AiCatalogFilterBlueprint = createExtensionBlueprint({
  ...
  attachTo: { id: 'page:ai-catalog/ai-catalog', input: 'filters' },
  ...
  *factory(params: {
    ...
    if (RESERVED_PARAMS.includes(params.urlParam)) {
      throw new Error(
        `FilterDefinition urlParam '${params.urlParam}' is reserved. Avoid: ${RESERVED_PARAMS.join(', ')}`,
      );
    }
