...
/**
 * Parsed CLI options for the migration-readiness tool.
 *
 * @internal
 */
...
/** Set of flags that consume the next argument as their value. @internal */
 ...
 *
 * Supports both `--flag value` and `--flag=value` syntax. Reads the
 * `BACKSTAGE_TOKEN` environment variable as a fallback when `--token`
 * is not provided.
 *
 * Calls `process.exit(0)` for `--help` and `process.exit(1)` for
 * validation errors.
 *
 * @internal
 */
