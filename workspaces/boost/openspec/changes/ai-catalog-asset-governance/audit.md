...
> **Post-audit resolution (2026-09-04, after re-audit):** The CRITICAL (Decision 4↔Decision 5 posture trigger) and both WARNINGs were subsequently addressed in a follow-up commit — Decision 4 now derives the policy-change timestamp via config-change detection (no `AdminConfigService.setOverride()`), `tasks.md` adds §6.9 (`ai-catalog.admin` bypass) and rewrites §6.8, `audit-logging` spec fixes the posture-change actor/trigger, and `default-deny-config` spec adds the ingestion-time boundary comparison scenario. The SUGGESTION (Tier 1/Tier 2 field-list drift) was also resolved: the field lists in `design.md` (Decision 1 table + Decision 2), `graduated-visibility/spec.md` (intro, permission table, frontend scenario), and `tasks.md` §2.1 are now aligned to the normative Tier 1/Tier 2 scenario (`category`/`lifecycle stage`/`version count`/`tags`; Tier 2 adds `deployment parameters`). No findings remain open. **Re-run `/opsx:audit` to independently confirm.**
