...
- [ ] 3.7 Add `rhdh.io/ai-asset-version` annotation population — extract raw version via `extractVersion(entity)`, normalize via `normalizeAIAssetVersion()` (SDK-exported from RHDHPLAN-1507); missing/unrecognized → `"0.0.0-unknown"` (do not use a local `"unknown"` fallback)
