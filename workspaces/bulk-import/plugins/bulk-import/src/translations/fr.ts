...
const bulkImportTranslationFr = createTranslationMessages({
  ...
  messages: {
    ...
    // Pull request
    'pullRequest.title':
      'Ajouter le fichier de configuration catalog-info.yaml',
    'pullRequest.description':
      "Cette pull request ajoute un **fichier de métadonnées d'entité Backstage**\nà ce dépôt afin que le composant puisse\nêtre ajouté au [catalogue logiciel]({{catalogUrl}}).\nAprès que cette pull request sera fusionnée, le composant deviendra disponible.\nPour plus d'informations, lisez un [aperçu du catalogue logiciel Backstage](https://backstage.io/docs/features/software-catalog/).\nVoir le travail d'importation dans votre application [ici]({{importJobUrl}}).",
    ...
    'previewFile.pullRequest.useCodeOwnersFile':
      "Utiliser le fichier <em>CODEOWNERS</em> comme propriétaire d'entité",
