...
The NFS page is registered with an `if` predicate that checks for the `bulk.import` permission. When permission checks deny access, the page and sidebar entry are not registered for that session. NFS gates page and nav registration via the `if` predicate; shared route components (such as `AddRepositoriesPage`) and the legacy entry point still use `usePermission` for in-page checks.
