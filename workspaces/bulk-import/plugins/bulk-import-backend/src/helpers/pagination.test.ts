...
describe('pagination', () => {
  ...
  it.each([undefined, []])(
    'should returned an empty array if %s array is passed',
    arr => {
  ...
  it('should throw an error if requested page number is negative', () => {
    expect(() => paginateArray(testInput, -1, 3)).toThrow(
      'page must be >0. Got page=-1',
    );
  });

  it('should throw an error if requested page number is zero', () => {
    expect(() => paginateArray(testInput, 0, 3)).toThrow(
      'page must be >0. Got page=0',
    );
  });

  it('should throw an error if requested page size is negative', () => {
    expect(() => paginateArray(testInput, 1, -1)).toThrow(
      'size must be >=0. Got size=-1',
    );
  });

  it.each([1, 2])(
    'should return empty array if requested page size is zero, regardless of page number (%d)',
    page => {
