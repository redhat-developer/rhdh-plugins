...
- Priority-ordered sidebar (`appSidebarExtension`, extension ID `nav-content:app/sidebar`) that renders `SidebarItemBlueprint`, `SidebarItemGroupBlueprint` and `SidebarElementBlueprint` contributions from `@red-hat-developer-hub/backstage-plugin-app-react`
