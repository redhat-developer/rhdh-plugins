...
function evaluateConditionObject(
  ...
  if (condition.isNotEmptyList !== undefined) {
    ...
    if (condition.isNotEmptyList ? !isNonEmptyList : isNonEmptyList) result = false;
  }

  // Check 'notContains' condition (hide if array does NOT contain value)
  if (condition.notContains !== undefined) {
    hasCondition = true;
    if (Array.isArray(fieldValue) && fieldValue.includes(condition.notContains)) result = false;
  }
