...
function isDatabaseService(opt) {
...
class DatabaseTaskStore {
  ...
  parseFilter(filter, query, db, negate = false) {
    ...
    return query[negate ? "andWhereNot" : "andWhere"]((subQuery) => {
      if (pluginPermissionNode.isOrCriteria(filter)) {
        for (const subFilter of filter.anyOf ?? []) {
          subQuery.orWhere(
            (subQueryInner) => this.parseFilter(subFilter, subQueryInner, db, false)
          );
        }
  ...
  async list(options) {
    const { createdBy, status, pagination, order, filters, permissionFilters } = options ?? {};
    const queryBuilder = this.db("tasks");
    const createdByValues = helpers.flattenParams(
      createdBy,
      filters?.createdBy
    );
    const combinedPermissionFilters = createdByValues.length > 0 ? {
      allOf: [
  ...
  async retryTask(options) {
    ...
    await this.db.transaction(async (tx) => {
      const result = await tx("tasks").where("id", taskId).update(
        {
