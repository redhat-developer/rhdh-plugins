...
async function getManifestByVersion(options) {
  ...
  const res = await withFallback(
    (signal) => fetchFn(`${versionsHost}/v1/releases/${versionEnc}/manifest.json`, {
      signal
    }),
    (signal) => fetchFn(`${gitHubRawBaseUrl}/v1/releases/${versionEnc}/manifest.json`, {
      signal
    }),
    500
  );
...
async function getManifestByReleaseLine(options) {
  ...
  const gitHubRawBaseUrl = options.gitHubRawBaseUrl ?? GITHUB_RAW_BASE_URL;
  const res = await withFallback(
    (signal) => fetchFn(`${versionsHost}/v1/tags/${releaseEnc}/manifest.json`, {
      signal
    }),
    async (signal) => {
...
export { getManifestByReleaseLine, getManifestByVersion, withFallback };
//# sourceMappingURL=manifest.esm.js.map
