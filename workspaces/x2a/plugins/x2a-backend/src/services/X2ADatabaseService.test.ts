  ...
  describe('createJob', () => {
    it.each(supportedDatabaseIds)(
      ...
      async databaseId => {
        ...
        expect(job.k8sJobName).toBeNull();
        expect(job.callbackToken).toBeNull();
        ...
        expect(row.k8s_job_name).toBeNull();
        expect(row.callback_token).toBeNull();
      },
    );
    ...
    it.each(databases.eachSupportedId())(
      'should create a job with specified phase - %p',
      async databaseId => {
        const { client } = await createDatabase(databaseId);
        const service = createService(client);
        ...
        expect(analyzeJob.phase).toBe('analyze');
        expect(migrateJob.phase).toBe('migrate');
        expect(publishJob.phase).toBe('publish');

        // Verify in database
        const analyzeRow = await client('jobs')
          .where('id', analyzeJob.id)
          .first();
        const migrateRow = await client('jobs')
          .where('id', migrateJob.id)
          .first();
        const publishRow = await client('jobs')
          .where('id', publishJob.id)
          .first();

        expect(analyzeRow.phase).toBe('analyze');
        expect(migrateRow.phase).toBe('migrate');
        expect(publishRow.phase).toBe('publish');
      },
    );

    it.each(databases.eachSupportedId())(
      'should create a job with errorDetails - %p',
      async databaseId => {
        const { client } = await createDatabase(databaseId);
        const service = createService(client);
        ...
        const errorDetails = 'Failed to connect to LLM service: timeout';
        const job = await service.createJob({
          moduleId: module.id,
          errorDetails,
        });

        expect(job.errorDetails).toBe(errorDetails);

        const row = await client('jobs').where('id', job.id).first();
        expect(row.error_details).toBe(errorDetails);
      },
    );

    it.each(databases.eachSupportedId())(
      'should create a job with k8sJobName - %p',
      async databaseId => {
        const { client } = await createDatabase(databaseId);
        const service = createService(client);
        ...
        const k8sJobName = 'job-x2a-init-abc123';
        const job = await service.createJob({
          moduleId: module.id,
          k8sJobName,
        });

        expect(job.k8sJobName).toBe(k8sJobName);

        const row = await client('jobs').where('id', job.id).first();
        expect(row.k8s_job_name).toBe(k8sJobName);
      },
    );

    it.each(databases.eachSupportedId())(
      'should create a job with callbackToken - %p',
      async databaseId => {
        const { client } = await createDatabase(databaseId);
        const service = createService(client);
        ...
        const callbackToken = 'callback-token-uuid-12345';
        const job = await service.createJob({
          moduleId: module.id,
          callbackToken,
        });

        expect(job.callbackToken).toBe(callbackToken);

        const row = await client('jobs').where('id', job.id).first();
        expect(row.callback_token).toBe(callbackToken);
      },
    );

    it.each(databases.eachSupportedId())(
      'should create a job with all new fields - %p',
      async databaseId => {
        const { client } = await createDatabase(databaseId);
        const service = createService(client);
        ...
        const job = await service.createJob({
          ...
          k8sJobName: 'job-x2a-migrate-xyz789',
          callbackToken: 'callback-token-xyz',
        });

        expect(job.phase).toBe('migrate');
        expect(job.errorDetails).toBe('Test error');
        expect(job.k8sJobName).toBe('job-x2a-migrate-xyz789');
        expect(job.callbackToken).toBe('callback-token-xyz');

        const row = await client('jobs').where('id', job.id).first();
        expect(row.phase).toBe('migrate');
        expect(row.error_details).toBe('Test error');
        expect(row.k8s_job_name).toBe('job-x2a-migrate-xyz789');
        expect(row.callback_token).toBe('callback-token-xyz');
      },
    );
  });
  ...
  describe('updateJob', () => {
    ...
    it.each(databases.eachSupportedId())(
      ...
      async databaseId => {
        ...
        expect(updated).toBeDefined();
        expect(updated?.phase).toBe('analyze');
    ...
    it.each(databases.eachSupportedId())(
      'should update job errorDetails - %p',
      async databaseId => {
        const { client } = await createDatabase(databaseId);
        const service = createService(client);
        ...
        const errorDetails =
          'Kubernetes job failed: ImagePullBackOff for x2a-convertor';
        const updated = await service.updateJob({
          id: job.id,
          errorDetails,
        });
        ...
        expect(updated).toBeDefined();
        expect(updated?.errorDetails).toBe(errorDetails);
    ...
    it.each(databases.eachSupportedId())(
      'should update job k8sJobName - %p',
      async databaseId => {
        const { client } = await createDatabase(databaseId);
        const service = createService(client);
        ...
        const k8sJobName = 'job-x2a-analyze-def456';
        const updated = await service.updateJob({
          id: job.id,
          k8sJobName,
        });
        ...
        expect(updated).toBeDefined();
        expect(updated?.k8sJobName).toBe(k8sJobName);
    ...
    it.each(databases.eachSupportedId())(
      'should update job callbackToken - %p',
      async databaseId => {
        const { client } = await createDatabase(databaseId);
        const service = createService(client);
        ...
        const callbackToken = 'new-callback-token-abc';
        const updated = await service.updateJob({
          id: job.id,
          callbackToken,
        });
        ...
        expect(updated).toBeDefined();
        expect(updated?.callbackToken).toBe(callbackToken);
    ...
    it.each(databases.eachSupportedId())(
      'should update all new job fields together - %p',
      async databaseId => {
        const { client } = await createDatabase(databaseId);
        const service = createService(client);
        ...
        const updated = await service.updateJob({
          id: job.id,
          phase: 'migrate',
          errorDetails: 'LLM rate limit exceeded',
          k8sJobName: 'job-x2a-migrate-ghi789',
          callbackToken: 'callback-token-final',
        });

        expect(updated).toBeDefined();
        expect(updated?.phase).toBe('migrate');
        expect(updated?.errorDetails).toBe('LLM rate limit exceeded');
        expect(updated?.k8sJobName).toBe('job-x2a-migrate-ghi789');
        expect(updated?.callbackToken).toBe('callback-token-final');

        const row = await client('jobs').where('id', job.id).first();
        expect(row.phase).toBe('migrate');
        expect(row.error_details).toBe('LLM rate limit exceeded');
        expect(row.k8s_job_name).toBe('job-x2a-migrate-ghi789');
        expect(row.callback_token).toBe('callback-token-final');
      },
    );
  });
