...
const x2aPluginTranslationJa = createTranslationMessages({
  ...
  messages: {
    ...
    'adversarialAgentsPage.notAllowed':
      '敵対的エージェントを管理する権限がありません。',
    'adversarialAgentsPage.subtitle':
      'セキュリティー、機能上のギャップ、および正確性の問題に関して移行出力をレビューする AI エージェントを管理します。',
    ...
    'editProjectDialog.ownerChangeWarning':
      'オーナーを変更すると、あなたの権限が新しいオーナーに適用されていない場合、このプロジェクトへのアクセス権を失う可能性があります。管理者は必要に応じてアクセス権を復元できます。',
    'editProjectDialog.ownerChangeWarningTitle': '所有権の譲渡を確認する',
    'editProjectDialog.ownerFormatHint':
      'Backstage エンティティー参照である必要があります (例: user:default/name または group:default/team)',
    ...
    'modulePage.phases.adversarialAgents.loadingError':
      '敵対的エージェントの読み込みに失敗しました',
    'modulePage.phases.adversarialAgents.noAgentsAvailable':
      'このフェーズでは、敵対的エージェントは利用できません。敵対的エージェントのページでエージェントを作成します。',
    'modulePage.phases.adversarialAgents.placeholder':
      'エージェントを選択します…',
    'modulePage.phases.adversarialAgents.title': '敵対的エージェント',
    'modulePage.phases.adversarialCriticalAgentsWarning':
      '重大な敵対的エージェントは設定済みですが、このフェーズでは実行されていません。',
    ...
    'modulePage.phases.staleInstructions':
      'アップストリームのフェーズが再実行されたため、このフェーズの結果は古くなっています。更新するには再実行します。',
    ...
    'adversarialAgentsPage.dialog.criticalHelper':
      '重大エージェントは重大レベルの検出を、非重大エージェントは警告レベルの検出を生成します',
    ...
    'adversarialAgentsPage.dialog.nameValidation':
      '名前は 3 文字以上 100 文字以内で入力する必要があります',
    ...
    'adversarialAgentsPage.dialog.phasesHelper':
      'このエージェントが実行されるワークフローフェーズを選択します',
    ...
    'adversarialAgentsPage.dialog.promptHelper':
      '調査対象と調査結果の報告方法について具体的に記述してください (50-5000 文字)',
    'adversarialAgentsPage.dialog.promptPlaceholder':
      'このエージェントが確認すべき項目を記述してください...',
    ...
    'resyncMigrationPlan.confirm.message':
      'これにより、移行計画が対象リポジトリーから再読み込みされ、それに応じてモジュールリストが更新されます。新しいモジュールが追加され、計画に含まれなくなったモジュールは削除済みとしてマークされます。モジュールを削除するなど、ドキュメントに変更を加える場合は、ドキュメントの整合性が保たれていることを確認してください。',
    'resyncMigrationPlan.confirm.warning':
      '削除済みとしてマークされたモジュールは、ジョブ履歴を保持しますが、新たなフェーズ実行の対象から外れます。削除されたモジュールについては、移行計画に再度追加しない限り、この操作を取り消すことはできません。',
    ...
    'rulesPage.deleteConfirm.message':
      'このルールは完全に削除されます。このルールをすでに受け入れている既存プロジェクトには影響はありません。',
