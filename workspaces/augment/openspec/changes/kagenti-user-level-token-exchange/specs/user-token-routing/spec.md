...
Acquisition and forwarding of user OIDC tokens via configurable request headers.

> **Follow-up:** Frontend OIDC token acquisition via `useApiHolder()` (modeled on the orchestrator's `findCustomProvider` pattern) is planned as a separate change. When implemented, the header-based path below will serve as the fallback for non-OIDC deployments.
...
#### Scenario: Chat route with OIDC token in header
...
#### Scenario: Chat route with no OIDC token
