...
describe('formatResponseText', () => {
  ...
  describe('markdown early-exit', () => {
    it('should skip heuristics when text has headings and bold', () => {
      const input =
        '# Title\n\n**SecureSSO & Certificate Management**\n- Gather info\n- Build payload';
      expect(formatResponseText(input)).toBe(input);
    });

    it('should skip heuristics when text has bold and lists', () => {
      const input =
        '**ServiceNow**\n- Retrieve tickets\n- Search by status\nname: John\nage: 30\ncity: NYC';
      expect(formatResponseText(input)).toBe(input);
    });

    it('should skip heuristics when text has headings and code blocks', () => {
      const input =
        '## Example\n\n```python\nprint("hello")\n```\n\nSome text.';
      expect(formatResponseText(input)).toBe(input);
    });

    it('should not skip for plain text without markdown markers', () => {
  ...
  describe('JSON formatting', () => {
    it('should format valid single-line JSON objects', () => {
    ...
    it('should format valid single-line JSON arrays', () => {
    ...
    it('should NOT match multi-line JSON across lines', () => {
      const input = 'Some text with {\n  "key": "value"\n} here';
      // The new regex only matches single-line JSON, so { and } on
      // separate lines should not be matched.
      expect(formatResponseText(input)).not.toContain('```json');
    });

    it('should NOT wrap markdown brackets in code blocks', () => {
      const input = 'See [reference](https://example.com) for details.';
      expect(formatResponseText(input)).toBe(input);
    });
  });
