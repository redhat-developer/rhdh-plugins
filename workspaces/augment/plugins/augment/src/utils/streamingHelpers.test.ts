...
import { createInitialStreamingState } from '../components/StreamingMessage';
...
describe('streamingHelpers', () => {
  ...
  describe('buildBotResponse - agent fields', () => {
    it('includes agentName from currentAgent', () => {
      const state = {
        ...createInitialStreamingState(),
        phase: 'completed' as const,
        completed: true,
        text: 'Hello from billing',
        currentAgent: 'Billing Agent',
      };

      const msg = buildBotResponse(state, 'msg-1');

      expect(msg.agentName).toBe('Billing Agent');
      expect(msg.handoffPath).toBeUndefined();
    });

    it('builds handoffPath from handoffs array', () => {
      const state = {
        ...createInitialStreamingState(),
        phase: 'completed' as const,
        completed: true,
        text: 'Resolved by support',
        currentAgent: 'Support Agent',
        handoffs: [
          { from: 'Triage Agent', to: 'Billing Agent' },
          { from: 'Billing Agent', to: 'Support Agent' },
        ],
      };

      const msg = buildBotResponse(state, 'msg-2');

      expect(msg.agentName).toBe('Support Agent');
      expect(msg.handoffPath).toEqual([
        'Triage Agent',
        'Billing Agent',
        'Support Agent',
      ]);
    ...
    it('omits agent fields when no agent is set (single-agent mode)', () => {
      const state = {
        ...createInitialStreamingState(),
        phase: 'completed' as const,
        completed: true,
      ...
      const msg = buildBotResponse(state, 'msg-3');

      expect(msg.agentName).toBeUndefined();
      expect(msg.handoffPath).toBeUndefined();
    });

    it('builds handoffPath with single handoff', () => {
      const state = {
        ...createInitialStreamingState(),
        phase: 'completed' as const,
        completed: true,
        text: 'Billing response',
        currentAgent: 'Billing Agent',
        handoffs: [{ from: 'Triage Agent', to: 'Billing Agent' }],
      };

      const msg = buildBotResponse(state, 'msg-4');

      expect(msg.handoffPath).toEqual(['Triage Agent', 'Billing Agent']);
    });
  });
