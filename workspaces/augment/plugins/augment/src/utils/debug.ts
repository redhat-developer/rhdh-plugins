...
/**
 * Debug logger for Augment frontend
 *
 * Controlled via:
 * - localStorage.setItem('augment.debug', 'true') - enables debug logging
...
const DEBUG_KEY = 'augment.debug';
const PREFIX = '[Augment]';
 ...
 * Called by AugmentApiClient when it has access to configApi
 *
 * @param enabled - Value from augment.debug in app-config.yaml
 */
 ...
 * 2. app-config.yaml augment.debug setting
 ...
 * Usage: window.enableAugmentDebug()
 */
 ...
 * Usage: window.disableAugmentDebug()
 */
...
if (typeof window !== 'undefined') {
  (window as unknown as Record<string, unknown>).enableAugmentDebug =
    enableDebug;
  (window as unknown as Record<string, unknown>).disableAugmentDebug =
    disableDebug;
}
