...
describe('adminEndpoints', () => {
  ...
  describe('Admin Config', () => {
    describe('getEffectiveConfig', () => {
      it('should fetch effective config', async () => {
        ...
        const mockConfig = { model: 'llama', baseUrl: 'https://test' };
  ...
  describe('Models', () => {
    describe('listModels', () => {
      it('should fetch models from backend', async () => {
        ...
        expect(deps.discoveryApi.getBaseUrl).toHaveBeenCalledWith('augment');
    ...
    describe('testMcpConnection', () => {
      it('should test MCP connection with url and type', async () => {
        ...
        expect(deps.fetchJson).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            body: JSON.stringify({ url: 'https://mcp:8080', type: 'stdio' }),
          }),
        );
        expect(result).toEqual(mockResult);
      });
      ...
      it('should include headers when provided', async () => {
        ...
        await testMcpConnection(deps, 'https://mcp', 'stdio', {
  ...
  describe('Workflows', () => {
    ...
    describe('getPromptGroups', () => {
      it('should fetch prompt groups', async () => {
        const deps = createDeps();
        const mockGroups = [{ id: 'g1', title: 'Group 1', cards: [] }];
        (deps.fetchJson as jest.Mock).mockResolvedValue({
          promptGroups: mockGroups,
        });

        const result = await getPromptGroups(deps);

        expect(deps.fetchJson).toHaveBeenCalledWith('/prompt-groups');
