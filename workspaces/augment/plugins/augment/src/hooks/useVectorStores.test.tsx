...
describe('useVectorStores', () => {
  ...
  it('populates unconnectedStores from API response', async () => {
    const unconnected = [
      {
        id: 'vs_unconnected',
    ...
    const api = {
      listActiveVectorStores: jest
        .fn()
        .mockResolvedValue({ stores: mockStores, unconnected }),
    };
    ...
    expect(result.current.unconnectedStores).toHaveLength(1);
    expect(result.current.unconnectedStores[0].id).toBe('vs_unconnected');
    expect(result.current.unconnectedStores[0].active).toBe(false);
  });

  it('clears unconnectedStores when API returns empty array', async () => {
    const api = {
      listActiveVectorStores: jest
        .fn()
        .mockResolvedValue({ stores: mockStores, unconnected: [] }),
    ...
    expect(result.current.unconnectedStores).toHaveLength(0);
  });

  it('connectStore calls API and refreshes', async () => {
    const api = {
      listActiveVectorStores: jest
        .fn()
        .mockResolvedValueOnce({ stores: mockStores })
      ...
      connectVectorStore: jest.fn().mockResolvedValue({
        activeVectorStoreIds: ['vs_1', 'vs_2', 'vs_new'],
      }),
    };
    ...
    await act(async () => {
      await result.current.connectStore('vs_new');
    });

    expect(api.connectVectorStore).toHaveBeenCalledWith('vs_new');
    expect(api.listActiveVectorStores).toHaveBeenCalledTimes(2);
  });
});
