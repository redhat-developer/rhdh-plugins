...
describe('useChatKeyboardShortcuts', () => {
  ...
  it('does NOT call cancelRequest when Escape is pressed and approval dialog is open', () => {
    const cancelRequest = jest.fn();
    const chatInputRef = createChatInputRef();
  ...
  it('does NOT call onNewChat when Cmd+Shift+O is pressed while streaming', () => {
    const onNewChat = jest.fn();
    const chatInputRef = createChatInputRef();
