...
function createWrapper(api: Partial<AugmentApi>) {
...
describe('useAdminConfig', () => {
  it('starts in loading state', () => {
    const api: Partial<AugmentApi> = {
      getAdminConfig: jest.fn(
        () => new Promise<{ entry: null; source: 'default' }>(() => {}),
      ),
    };

    const { result } = renderHook(() => useAdminConfig('promptGroups'), {
  ...
  it('loads config entry successfully', async () => {
    const mockEntry = {
      configKey: 'promptGroups' as const,
      configValue: [{ id: 'g1', title: 'Group 1', cards: [] }],
