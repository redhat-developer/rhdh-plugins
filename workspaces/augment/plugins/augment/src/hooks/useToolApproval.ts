...
function generateMessageId(): string {
  const suffix =
    typeof globalThis.crypto?.randomUUID === 'function'
      ...
      : Array.from(
          { length: 8 },
          () => ((16 * Math.random()) | 0).toString(16), // NOSONAR — display-only ID
        ).join('');
      ...
      try {
        ...
        if (result.success) {
          ...
          const approvedToolCall: {
            ...
            arguments: toolArgs ?? pendingApproval.toolCall.arguments,
          ...
          if (result.outputTruncated) {
            finalText +=
              '\n\n> **Note:** The tool output was truncated to fit within context limits. You may want to narrow your query for complete results.';
          }

          if (result.handoff) {
            finalText += `\n\n---\n*Handed off from **${result.handoff.fromAgent}** to **${result.handoff.toAgent}***`;
          }
