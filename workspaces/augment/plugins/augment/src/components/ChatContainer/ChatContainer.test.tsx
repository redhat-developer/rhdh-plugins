...
describe('ChatContainer', () => {
  ...
  it('shows empty session state when activeSessionId is set but no messages', () => {
    const api = createFullMockApi();

    render(<ChatContainer {...defaultProps} activeSessionId="session-1" />, {
      wrapper: createWrapper(api),
    });

    expect(screen.getByText(/Start a conversation/i)).toBeInTheDocument();
    expect(
      screen.getByText(/Ask anything — I'm here to help\./i),
    ).toBeInTheDocument();
  });

  it('shows messages unavailable state when messagesUnavailable is true', () => {
    const api = createFullMockApi();

    render(
      <ChatContainer
        {...defaultProps}
        activeSessionId="session-lost"
        messagesUnavailable
      />,
      { wrapper: createWrapper(api) },
    );
    ...
    expect(
      screen.getByText(/Previous messages are no longer available/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/conversation history may have expired/i),
    ).toBeInTheDocument();
  });

  it('shows welcome screen when no messages and no activeSessionId', () => {
    const api = createFullMockApi();
    ...
    // Welcome screen renders (verified by the presence of the input placeholder)
    // and the empty session text is NOT shown
