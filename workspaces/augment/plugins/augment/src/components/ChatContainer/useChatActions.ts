...
export function useChatActions({
  ...
  const handleRegenerate = useCallback(() => {
    ...
    void sendMessage(lastUserMessage.text, kept);
  }, [messages, onMessagesChange, sendMessage, cancelRequest]);
  ...
  const handleEditMessage = useCallback(
    (messageId: string, newText: string) => {
      ...
      void sendMessage(newText, truncated);
    },
    [messages, onMessagesChange, sendMessage, cancelRequest],
  );
  ...
  const handleSendMessage = useCallback(() => {
    if (inputValue.trim()) {
      ...
      setInputValue('');
      void sendMessage(messageText, messages);
    }
