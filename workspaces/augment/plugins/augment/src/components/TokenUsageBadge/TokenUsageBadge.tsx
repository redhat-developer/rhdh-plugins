...
export const TokenUsageBadge = ({ usage }: TokenUsageBadgeProps) => {
  ...
  const tooltipContent = useMemo(() => {
    ...
    if (cachedTokens > 0)
      lines.push(`Cached: ${formatTokenCount(cachedTokens)}`);
    if (reasoningTokens > 0)
      lines.push(`Reasoning: ${formatTokenCount(reasoningTokens)}`);
    lines.push('', 'Token usage reported by inference server');
    return lines.join('\n');
