...
function summarizeToolNames(toolCalls: ToolCallInfo[]): string {
  const counts = new Map<string, number>();
  for (const t of toolCalls) {
    const name = stripToolPrefix(t.name, t.serverLabel);
    ...
    >
      ...
      >
        ...
        >
          ...
          Used {toolCalls.length} tool call{toolCalls.length > 1 ? 's' : ''}
          {' \u2014 '}
          {summarizeToolNames(toolCalls)}
        </Typography>
