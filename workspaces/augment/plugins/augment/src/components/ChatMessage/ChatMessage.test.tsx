...
const mockApi: Partial<AugmentApi> = {
...
describe('ChatMessage', () => {
  ...
  describe('token usage', () => {
    ...
    it('should not display token usage badge when usage is absent', () => {
      renderChatMessage(baseAssistantMessage);
      expect(screen.queryByText(/\d{1,20} in$/)).not.toBeInTheDocument();
    });
  ...
  describe('handoff path breadcrumb', () => {
    it('renders handoff path when message has multiple entries', () => {
      const messageWithHandoffs: Message = {
        ...baseAssistantMessage,
        handoffPath: ['Triage Agent', 'Billing Agent', 'Technical Agent'],
      };
      renderChatMessage(messageWithHandoffs);
      expect(
        screen.getByText(
          'Triage Agent \u2192 Billing Agent \u2192 Technical Agent',
        ),
      ).toBeInTheDocument();
    });

    it('does not render handoff path when only one entry', () => {
      const messageWithSingleAgent: Message = {
        ...baseAssistantMessage,
        handoffPath: ['Billing Agent'],
      };
      renderChatMessage(messageWithSingleAgent);
      expect(screen.queryByText(/\u2192/)).not.toBeInTheDocument();
    });

    it('does not render handoff path when not present', () => {
      renderChatMessage(baseAssistantMessage);
      expect(screen.queryByText(/\u2192/)).not.toBeInTheDocument();
    });

    it('does not render handoff path for user messages', () => {
      const userWithPath: Message = {
        ...baseUserMessage,
        handoffPath: ['Agent A', 'Agent B'],
      };
      renderChatMessage(userWithPath);
      expect(screen.queryByText(/\u2192/)).not.toBeInTheDocument();
    });

    it('renders handoff path with exactly 2 entries', () => {
      const msg: Message = {
        ...baseAssistantMessage,
        handoffPath: ['Triage', 'Billing'],
      };
      renderChatMessage(msg);
      expect(screen.getByText('Triage \u2192 Billing')).toBeInTheDocument();
    });

    it('filters out empty strings from handoff path', () => {
      const msg: Message = {
        ...baseAssistantMessage,
        handoffPath: ['Agent A', '', 'Agent B'],
      };
    ...
    it('has accessible aria-label on handoff breadcrumb', () => {
      const msg: Message = {
        ...baseAssistantMessage,
        handoffPath: ['Triage', 'Billing'],
      };
      renderChatMessage(msg);
      expect(
        screen.getByLabelText(/Handoff path: Triage to Billing/),
      ).toBeInTheDocument();
    });
  });
