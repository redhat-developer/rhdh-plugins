 ...
 */
export function AdminLayout({ adminScrollSx }: AdminLayoutProps) {
  const {
    liveStatus,
    adminPanel,
    setAdminPanel,
  ...
  // Sub-tab groups: which panels belong to which top-level tab
  const AGENTS_PANELS: AdminPanel[] = ['ops-review-queue', 'ops-registry', 'kagenti-builds'];
  const PLATFORM_PANELS: AdminPanel[] = ['kagenti-tools', 'ops-tool-review', 'ops-platform', 'ops-observability'];
  ...
  const activeTopTab = useMemo(() => {
    if (adminPanel === 'ops-home' || adminPanel === 'kagenti-home') return 'ops-home';
  ...
  const handleTopTabChange = useCallback((panel: AdminPanel) => {
    // When switching top tabs, go to the first sub-panel of that group
    if (panel === 'ops-review-queue' && !AGENTS_PANELS.includes(adminPanel)) setAdminPanel('ops-review-queue');
    else if (panel === 'ops-platform' && !PLATFORM_PANELS.includes(adminPanel)) setAdminPanel('ops-platform');
    else if (panel === 'ops-branding' && !SETTINGS_PANELS.includes(adminPanel)) setAdminPanel('ops-branding');
      ...
      <TourControllerProvider callbacks={tourCallbacks}>
        <TourProvider>
          <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, minWidth: 0 }}>
            ...
            >
              <Box sx={{ maxWidth: 960, width: '100%', minWidth: 0, px: { xs: 2, sm: 4, md: 5 }, py: 3 }}>
                {/* Sub-tabs -- same container as content, same left edge */}
                {AGENTS_PANELS.includes(adminPanel) && (
                  <Box sx={{ display: 'flex', gap: 0.5, mb: 2.5, pb: 1.5, borderBottom: 1, borderColor: 'divider', alignItems: 'center' }}>
                    {[
                      { id: 'ops-review-queue' as AdminPanel, label: 'Review Queue' },
                      { id: 'ops-registry' as AdminPanel, label: 'Registry' },
                      { id: 'kagenti-builds' as AdminPanel, label: 'Builds' },
                    ].map(t => (
                      <Box
                        key={t.id}
                        ...
                        sx={{
                          ...
                          fontSize: '0.78rem', fontWeight: adminPanel === t.id ? 700 : 500,
                          color: adminPanel === t.id ? 'primary.main' : 'text.secondary',
                          bgcolor: adminPanel === t.id ? 'action.selected' : 'transparent',
                    ...
                    {namespacePicker && <Box sx={{ ml: 'auto', maxWidth: 180 }}>{namespacePicker}</Box>}
                  </Box>
                )}
                {PLATFORM_PANELS.includes(adminPanel) && (
                  <Box sx={{ display: 'flex', gap: 0.5, mb: 2.5, pb: 1.5, borderBottom: 1, borderColor: 'divider', alignItems: 'center' }}>
                    {[
                      { id: 'kagenti-tools' as AdminPanel, label: 'Tools' },
                      { id: 'ops-tool-review' as AdminPanel, label: 'Tool Review' },
                      { id: 'ops-platform' as AdminPanel, label: 'Config' },
                      { id: 'ops-observability' as AdminPanel, label: 'Observability' },
                    ].map(t => (
                      <Box
                        key={t.id}
                        onClick={() => setAdminPanel(t.id)}
                ...
                {SETTINGS_PANELS.includes(adminPanel) && (
                  <Box sx={{ display: 'flex', gap: 0.5, mb: 2.5, pb: 1.5, borderBottom: 1, borderColor: 'divider' }}>
                    {[
                      { id: 'ops-branding' as AdminPanel, label: 'Branding' },
                      { id: 'ops-admin' as AdminPanel, label: 'Administration' },
                      { id: 'ops-docs' as AdminPanel, label: 'Documentation' },
                    ].map(t => (
                      <Box
                        key={t.id}
                        onClick={() => setAdminPanel(t.id)}
