...
export function WorkflowCodeExport({
  workflow,
  onClose,
}: WorkflowCodeExportProps) {
  ...
  const backendUrl =
    configApi.getOptionalString('augment.llamaStack.baseUrl') ||
    'http://localhost:8321';
  const model =
    configApi.getOptionalString('augment.llamaStack.model') ||
    'meta-llama/Llama-3.3-70B-Instruct';

  const tsCode = useMemo(
    () =>
      generateWorkflowCode(workflow, {
        llamaStackUrl: `${backendUrl}/v1`,
        defaultModel: model,
      }),
    [workflow, backendUrl, model],
  );
  ...
  if (topTab === 0) {
    currentCode = sdkLang === 0 ? tsCode : pyCode;
  } else {
  ...
  if (topTab === 0) {
    currentLang = sdkLang === 0 ? 'typescript' : 'python';
  } else {
    currentLang = 'json';
  }
  ...
  const handleDownload = useCallback(() => {
    ...
    a.href = url;
    a.download = `${workflow.name.replace(/\s+/g, '-').toLowerCase()}.${currentExt}`;
    a.click();
    URL.revokeObjectURL(url);
  }, [currentCode, currentExt, workflow.name]);

  const copyWorkflowId = useCallback(() => {
    window.navigator.clipboard.writeText(workflow.id);
    setIdCopied(true);
    ...
    >
      ...
      {topTab === 1 ? (
        ...
        <Box sx={{ flex: 1, overflowY: 'auto', px: 3, py: 2.5 }}>
          ...
          >
            Use the workflow ID below to integrate this agent workflow into your
            application via the Responses API.
          </Typography>
          ...
          >
            Use the Agents SDK to run this workflow programmatically, or export
            the workflow definition as JSON for external systems.
          </Typography>
