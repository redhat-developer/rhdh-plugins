...
function generateAgentPy(node: WorkflowNode, defaultModel: string): string {
  ...
  const instructions =
    (d.instructions as string) || 'You are a helpful assistant.';
  ...
  if (temp !== undefined || maxTokens !== undefined || reasoning) {
    ...
    if (reasoning)
      code += `        reasoning=ReasoningEffort(effort="${reasoning}"),\n`;
...
export function generatePythonCode(
  ...
  for (const cn of classifyNodes) {
    ...
    const classifications =
      (d.classifications as Array<{ label: string }>) || [];
    const varName = pyVarName((d.label as string) || cn.id);
    lines.push(`# Classify node: ${varName}`);
    lines.push(
      `${varName}_labels = [${classifications.map(c => `"${pyStr(c.label)}"`).join(', ')}]`,
    );
    lines.push(``);
  }
  ...
  if (agents.length > 0) {
    ...
    const agentVar = pyVarName(
      (d.name as string) || (d.agentKey as string) || firstAgent.id,
    );
    lines.push(
      `        result = await runner.run(${agentVar}, conversation_history)`,
    );
    lines.push(``);
    ...
    for (let i = 1; i < agents.length; i++) {
      ...
      const av = pyVarName(
        (ad.name as string) || (ad.agentKey as string) || a.id,
      );
      lines.push(``);
      lines.push(
        `        result = await runner.run(${av}, conversation_history)`,
      );
      lines.push(`        conversation_history.extend(`);
      lines.push(`            [item.raw_item for item in result.new_items]`);
      lines.push(`        )`);
