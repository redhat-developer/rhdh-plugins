...
export function WorkflowEditor({
  ...
  const handleGraphChange = useCallback(
    (nodes: WorkflowNode[], edges: WorkflowEdge[]) => {
      ...
      if (removed.length > 0 || edges.length !== workflow.edges.length) {
        onSave({
          ...workflow,
          nodes: merged.filter(n => newNodeIds.includes(n.id)),
          edges,
          updatedAt: new Date().toISOString(),
        });
      } else {
  ...
  const handleSecretsChange = useCallback(
    (secrets: Array<{ name: string; value: string }>) => {
      onSave({
        ...workflow,
        settings: { ...workflow.settings, secrets },
        updatedAt: new Date().toISOString(),
  ...
  const secrets =
    ((workflow.settings as Record<string, unknown>)?.secrets as Array<{
      name: string;
      value: string;
    }>) || [];
  ...
  return (
    ...
    >
      ...
      >
        ...
        {mode === 'edit' && !readOnly && !isNarrow && (
          <AgentListPanel onAddNode={handleAddNode} />
        )}
        ...
        >
          ...
          <Slide
            direction="left"
            in={mode === 'edit' && !!selectedNode && !readOnly}
            mountOnEnter
            unmountOnExit
          >
            <Box
      ...
      <Dialog
        ...
        fullWidth
        PaperProps={{
          sx: { height: '80vh', maxHeight: '80vh', borderRadius: 2 },
        }}
      >
        <WorkflowCodeExport
          workflow={workflow}
        ...
        <WorkflowEvaluation
          ...
          onRunEvaluation={async (testCases, scoringFunctions) => {
            ...
            const resp = await authFetch(
              `${backendUrl}/api/augment/workflows/${workflow.id}/evaluate`,
              {
                method: 'POST',
