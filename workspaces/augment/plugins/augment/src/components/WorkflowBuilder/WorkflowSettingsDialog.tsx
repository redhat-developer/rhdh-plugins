...
export function WorkflowSettingsDialog({
  ...
  const copyId = useCallback(() => {
    window.navigator.clipboard.writeText(workflowId);
    setCopied(true);
  ...
  const updateSecret = useCallback(
    (index: number, field: keyof Secret, val: string) => {
  ...
  return (
    ...
    >
      <DialogContent sx={{ p: 3 }}>
        ...
        >
          Write-only secrets for this workflow. Refer to them by name in MCP
          nodes and the Agent node MCP tool for authentication and custom
          headers (for example, {'{{MY_SECRET}}'}).
        </Typography>
