...
export function CollapsedSidebar({
  ...
  return (
    ...
    >
      <Tooltip
        title={`Agent: ${overallReady ? t('agentInfo.ready') : t('agentInfo.offline')}`}
        placement="left"
      >
        <Box
          sx={{
      ...
      {isAdmin && onAdminClick && (
        <Box sx={{ mt: 'auto', mb: 1 }}>
          <Tooltip title={t('commandCenter.title')} placement="left">
            <IconButton
              size="small"
              onClick={onAdminClick}
              aria-label={t('rightPane.openCommandCenter')}
              sx={{
