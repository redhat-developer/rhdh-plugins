...
export function registerCrudRoutes(
  ...
  router.get(
    ...
    requireAdminAccess,
    withRoute(
      req => `Get workflow ${req.params.id}`,
    ...
    withRoute(
      ...
      async (req, res) => {
        ...
        if (!body.id || !body.name)
          throw new InputError('Workflow must have an id and name');
  ...
  router.put(
    ...
    requireAdminAccess,
    withRoute(
      req => `Update workflow ${req.params.id}`,
        ...
        try {
          res.json(
            await workflowService.updateWorkflow(
              req.params.id,
              req.body as Partial<WorkflowDefinition>,
              user,
            ),
          );
        } catch (err) {
  ...
  router.delete(
    ...
    requireAdminAccess,
    withRoute(
      req => `Delete workflow ${req.params.id}`,
      'Failed to delete workflow',
      async (req, res) => {
        const user = await getUserRef(req);
        try {
          await workflowService.deleteWorkflow(req.params.id, user);
