  ...
  const mockProvider: Partial<AgenticProvider> = {
    ...
    rag: {
      ...
      generateAnswer: jest.fn().mockResolvedValue({
        query: 'test question',
        answer: 'Generated answer from chunks',
        chunks: [
          {
            text: 'Hello world chunk',
            score: 0.95,
...
describe('Admin routes', () => {
  ...
  describe('PUT /admin/config/:key', () => {
    it('sets a config value', async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.configKey).toBe('promptGroups');
    });
  ...
  describe('GET /admin/config/:key', () => {
    it('returns null for unset keys', async () => {
      ...
      const res = await request(app).get('/admin/config/promptGroups');
      expect(res.status).toBe(200);
  ...
  // ---------------------------------------------------------------------------
  // RAG Generate (end-to-end)
  // ---------------------------------------------------------------------------

  describe('POST /admin/rag-generate', () => {
    it('generates an answer from retrieved chunks', async () => {
      const { create } = createApp(db);
      const app = await create();
      ...
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.answer).toBe('Generated answer from chunks');
    ...
    it('returns 501 when generateAnswer is not available', async () => {
      const { create } = createApp(db, {
        providerPatch: {
          rag: {
            listDocuments: jest.fn().mockResolvedValue([]),
  ...
  describe('GET /admin/vector-store-config', () => {
    ...
    it('replaces embeddingModel when config model is not on server', async () => {
      const { create } = createApp(db, {
        providerPatch: {
          listModels: jest.fn().mockResolvedValue([
            { id: 'nomic-embed-v1.5', model_type: 'embedding' },
            { id: 'granite-llm', model_type: 'llm' },
          ]),
        },
      });
      const app = await create();
      ...
      const res = await request(app).get('/admin/vector-store-config');
      expect(res.status).toBe(200);
      expect(res.body.config.embeddingModel).toBe('nomic-embed-v1.5');
    });

    it('keeps embeddingModel when it exists on server', async () => {
      const { create } = createApp(db, {
        providerPatch: {
          listModels: jest.fn().mockResolvedValue([
            {
            ...
            { id: 'other-embed', model_type: 'embedding' },
          ]),
        },
      });
      const app = await create();
    ...
    it('keeps embeddingModel unchanged when listModels throws', async () => {
      const { create } = createApp(db, {
        providerPatch: {
          listModels: jest.fn().mockRejectedValue(new Error('Server down')),
        },
      });
      const app = await create();
  ...
  describe('GET /admin/vector-stores', () => {
    ...
    it('returns unconnected stores not in active list', async () => {
      const { create, provider } = createApp(db);
      (provider.rag!.listVectorStores as jest.Mock).mockResolvedValue([
        {
          id: 'vs_test',
      ...
      expect(res.body.stores).toHaveLength(1);
      expect(res.body.stores[0].id).toBe('vs_test');
      expect(res.body.unconnected).toHaveLength(1);
      expect(res.body.unconnected[0].id).toBe('vs_other');
      expect(res.body.unconnected[0].active).toBe(false);
    });
  });
  ...
  describe('DELETE /admin/vector-stores/:id', () => {
    ...
    it('still removes from active list when deleteVectorStore throws', async () => {
      const { create, provider } = createApp(db);
      (provider.rag!.deleteVectorStore as jest.Mock).mockRejectedValueOnce(
      ...
      expect(res.body.success).toBe(true);
      expect(res.body.removed).toBe('vs_test');
      expect(res.body.filesDeleted).toBe(0);
      expect(provider.rag!.removeVectorStoreId).toHaveBeenCalledWith('vs_test');
    });

    it('returns 501 when RAG is not available', async () => {
      const { create } = createApp(db, {
        providerPatch: { rag: undefined } as any,
      });
      const app = await create();
  ...
  describe('GET /admin/effective-config', () => {
    ...
    it('returns provider effective config when getEffectiveConfig is implemented', async () => {
      ...
      expect(res.body.config.baseUrl).toBe('https://localhost:8321');
  ...
  describe('GET /prompt-groups — DB + YAML round-trip', () => {
    it('returns YAML prompt groups when no DB override exists', async () => {
      const { create } = createApp(db);
      const app = await create();
    ...
    it('returns DB prompt groups after admin save', async () => {
      const { create } = createApp(db);
      const app = await create();
      ...
      expect(res.status).toBe(200);
      expect(res.body.source).toBe('database');
      expect(res.body.promptGroups).toEqual(lanes);
    });

    it('reverts to YAML after admin deletes prompt groups', async () => {
      const { create } = createApp(db);
      const app = await create();
  ...
  describe('MCP test-connection SSRF protection', () => {
    it.each([
      ...
      },
      {
        ...
        url: 'https://192.168.1.1:3000/mcp',
        errorMatch: 'private/internal network',
      },
      {
        desc: '172.16-31.x private network',
        url: 'https://172.20.0.1:3000/mcp',
        errorMatch: 'private/internal network',
      },
      {
