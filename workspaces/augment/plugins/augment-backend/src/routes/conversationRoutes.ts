...
import type { ConversationDetails } from '../providers/llamastack/conversationTypes';

/**
 * Strip system prompt / instructions from a ConversationDetails before
 * returning it to the frontend. Llama Stack may store instructions as
 * `input_text` content parts in output items or as developer/system
 * messages in the input array.
 */
function sanitizeConversationDetails(
  details: ConversationDetails,
): ConversationDetails {
  const sanitizedOutput = (details.output || []).map(item => {
    if (item.role !== 'assistant' || !Array.isArray(item.content)) return item;
    return {
      ...item,
      content: item.content.filter(c => c.type !== 'input_text'),
    };
  });

  let sanitizedInput = details.input;
  if (Array.isArray(details.input)) {
    sanitizedInput = (details.input as Array<Record<string, unknown>>).filter(
      item => {
  ...
  router.get(
    ...
    withRoute(
      ...
      async (req, res) => {
        ...
        const messages = chainResult.items.map(
  ...
  router.get(
    ...
    withRoute(
      ...
      async (req, res) => {
        ...
        res.json({
          success: true,
          conversation: sanitizeConversationDetails(conversation),
          timestamp: new Date().toISOString(),
        });
      },
    ),
  ...
  router.get(
    ...
    withRoute(
      ...
      async (req, res) => {
        ...
        const sanitizedItems = result.items.filter(
          (item: { role?: string }) => {
        ...
        logger.info(
          `Returning ${sanitizedItems.length} input items for ${responseId} (${result.items.length - sanitizedItems.length} system items stripped)`,
        );
        res.json({
          success: true,
  ...
  router.delete(
    ...
    withRoute(
      ...
      async (req, res) => {
        ...
        await provider.conversations!.delete(responseId, conversationId);
        res.json({
          success: true,
