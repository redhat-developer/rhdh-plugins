...
export function registerAdminRoutes(
  ...
  // ---------------------------------------------------------------------------
  // RAG Generate (end-to-end: retrieval + LLM answer)
  // ---------------------------------------------------------------------------
      ...
      async (req, res) => {
        ...
        try {
          ...
          if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
            throw new InputError('URL must use http:// or https:// protocol');
          }
        } catch (e) {
        ...
        if (ssrfReason) {
          throw new InputError(
            `URL points to a private/internal network address and is blocked for security (${ssrfReason})`,
          );
        }

        const skipTls =
          config.getOptionalBoolean('augment.llamaStack.skipTlsVerify') ??
          false;
