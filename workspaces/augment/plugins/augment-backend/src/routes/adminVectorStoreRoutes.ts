  ...
  router.get(
    ...
    withRoute(
      ...
      async (_req, res) => {
        ...
        let source: 'yaml' | 'merged' | 'database';

        if (resolved) {
          const dbOverrides = await adminConfig.getScopedValue(
            'vectorStoreConfig',
            provider.id as import('@red-hat-developer-hub/backstage-plugin-augment-common').ProviderType,
          );
          source = dbOverrides !== undefined ? 'merged' : 'yaml';
          cfgObj = { ...resolved } as Record<string, unknown>;
        } else {
          const ls = config.getOptionalConfig('augment.llamaStack');
          source = 'yaml';
          cfgObj = readVectorStoreDefaults(ls);
        }
        ...
        if (provider.listModels) {
          try {
            ...
            const currentModel = cfgObj.embeddingModel as string | undefined;
            if (
            ...
            logger.debug(
              `Could not validate embeddingModel against server models: ${err}`,
            );
          }
        }
    ...
    withRoute(
      ...
      async (_req, res) => {
        ...
        if (
          ...
          try {
            const discovered = await activeIdsMutex.run(async () => {
              const freshDb = await adminConfig.getScopedValue(
                'activeVectorStoreIds',
                providerId,
              );
              if (freshDb !== undefined) return null;
            ...
            if (discovered) {
              onConfigChanged?.();
              logger.info(
                `Auto-populated active vector store from lazy init: ${discovered[0]}`,
              );
              activeIds = discovered;
            }
          } catch {
            logger.warn(
      ...
      async (req, res) => {
        ...
        if (permanent && provider.rag.deleteVectorStore) {
          try {
            ...
            logger.warn(
              `Server-side delete failed for ${id}: ${
                deleteErr instanceof Error ? deleteErr.message : deleteErr
              }. Proceeding with local removal.`,
            );
          }
        }
