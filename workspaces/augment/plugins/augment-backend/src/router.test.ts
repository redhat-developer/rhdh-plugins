...
describe('createRouter', () => {
  ...
  describe('GET /prompt-groups', () => {
    it('returns empty prompt groups list', async () => {
      const response = await request(app).get('/prompt-groups');

      expect(response.status).toEqual(200);
      expect(response.body).toHaveProperty('success', true);
      expect(response.body).toHaveProperty('promptGroups', []);
    });
  });

  describe('GET /swim-lanes (removed)', () => {
    it('returns 404 for deprecated swim-lanes endpoint', async () => {
  ...
  describe('Session routes', () => {
    ...
    it('should get processed messages for a session', async () => {
      ...
      expect(response.body).toHaveProperty('hasConversationId', true);
    });

    it('should cascade-delete conversation container when deleting a session with conversationId', async () => {
      const sessionData = {
        id: '123',
        title: 'My Session',
        userRef: 'user:default/testuser',
      ...
      const mockGetSession = jest.fn().mockResolvedValue(sessionData);
      const mockDeleteSession = jest.fn().mockResolvedValue(true);
      const mockDeleteContainer = jest.fn().mockResolvedValue(true);
      const mockSessions = createMockSessions({
        getSession: mockGetSession,
        deleteSession: mockDeleteSession,
      });

      const providerWithDelete = {
        ...mockProvider,
        conversations: {
          ...mockProvider.conversations!,
          deleteContainer: mockDeleteContainer,
        },
      };

      const router = await createRouter(
        createTestRouterOptions(db, providerWithDelete, adminConfig, {
          sessions: mockSessions,
        }),
      );
      const appSessions = express();
      ...
      expect(response.status).toEqual(200);
      expect(response.body).toHaveProperty('success', true);
      expect(mockDeleteContainer).toHaveBeenCalledWith('conv-cascade');
      expect(mockDeleteSession).toHaveBeenCalledWith(
        '123',
        'user:default/testuser',
      );
    ...
    it('should return 502 when getProcessedMessages fails', async () => {
      const sessionData = {
        id: '123',
        title: 'My Session',
        userRef: 'user:default/testuser',
      ...
      (
        mockProvider.conversations!.getProcessedMessages as jest.Mock
      ).mockRejectedValue(new Error('Llama Stack unreachable'));
      const mockSessions = createMockSessions({
        getSession: mockGetSession,
      });
      ...
      expect(response.body).toHaveProperty(
        'error',
        'Failed to retrieve messages from the conversation provider',
      );
      expect(response.body).toHaveProperty('messages', []);
      expect(response.body).toHaveProperty('hasConversationId', true);
    });

    it('should return empty messages for session without conversationId', async () => {
      const sessionData = {
        id: '123',
        title: 'My Session',
        userRef: 'user:default/testuser',
      ...
      expect(response.body).toHaveProperty('hasConversationId', false);
    });
  });
});
