...
export function createSecurityMiddleware(deps: SecurityDeps) {
  ...
  const requirePluginAccess: express.RequestHandler = async (
    ...
    try {
      ...
      logger.debug(`Plugin access check: augment.access = ${decision.result}`);

      if (decision.result !== AuthorizeResult.ALLOW) {
        logger.warn(
          `Augment access denied for user (result=${decision.result})`,
        );
        const deniedMessage =
          securityConfig.accessDeniedMessage ||
          'You do not have permission to access Augment. ' +
   ...
   *  2. permission.enabled === true → delegate to Backstage RBAC
   *  3. Otherwise → check user entity ref against augment.security.adminUsers
   */
  async function checkIsAdmin(req: express.Request): Promise<boolean> {
      ...
      try {
        ...
        const decision = (
          await permissions.authorize(
            [{ permission: augmentAdminPermission }],
            { credentials },
          )
        )[0];
    ...
    try {
      ...
      logger.info(
        `checkIsAdmin: userRef="${userRef}", adminCount=${adminUsers.length}, match=${isMatch}`,
      );
      return isMatch;
    } catch {
      logger.debug('Permission check threw, treating as non-admin');
  ...
  const requireAdminAccess: express.RequestHandler = async (req, res, next) => {
    ...
    logger.warn('Augment admin access denied for user');
    res.status(403).json({
      error: 'Admin Access Denied',
      message:
        'You do not have admin permissions for Augment. ' +
