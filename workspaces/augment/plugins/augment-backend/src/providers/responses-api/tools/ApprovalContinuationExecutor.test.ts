...
describe('executeApprovalContinuation', () => {
  ...
  describe('1. Successful approval flow', () => {
    ...
    it('includes instructions and tool_choice when approved, excludes production params', async () => {
      const mockClient = createMockClient();
      mockClient.request.mockResolvedValue({
        id: 'resp-2',
        output: [{ type: 'mcp_call', name: 'projects_list', output: 'ok' }],
  ...
  describe('5. MCP tool building', () => {
    ...
    it('overrides require_approval to "never" in approval continuation even when server has "always"', async () => {
      const mockClient = createMockClient();
      mockClient.request.mockResolvedValue({
        id: 'resp-2',
        output: [{ type: 'mcp_call', name: 'projects_list', output: 'ok' }],
      ...
      const body = JSON.parse(mockClient.request.mock.calls[0][1].body);
      expect(body.tools[0].require_approval).toBe('never');
