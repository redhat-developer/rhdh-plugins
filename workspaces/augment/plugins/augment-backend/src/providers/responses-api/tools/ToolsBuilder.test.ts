...
import { buildTools, sanitizeToolsForServer } from './ToolsBuilder';
...
describe('buildTools', () => {
  ...
  it('uses direct server URL when mcpAuth is provided', async () => {
    const mcpAuth = {
      getApiApprovalConfig: jest.fn().mockReturnValue('never'),
...
describe('sanitizeToolsForServer', () => {
  ...
  const noStrictCaps: CapabilityInfo = {
    ...allCaps,
    strictField: false,
  };

  it('passes tools through when server supports strict', () => {
    ...
    const result = sanitizeToolsForServer(tools, allCaps, logger);
    expect(result).toEqual(tools);
  });

  it('strips strict field from function tools when server does not support it', () => {
    const tools: ResponsesApiTool[] = [
      {
        type: 'function',
        name: 'f1',
      ...
      { type: 'function', name: 'f2', description: 'd2', parameters: {} },
      { type: 'web_search' },
    ];
    const result = sanitizeToolsForServer(tools, noStrictCaps, logger);

    const f1 = result.find(
      t => t.type === 'function' && t.name === 'f1',
    ) as unknown as Record<string, unknown>;
    expect(f1).toBeDefined();
    expect(f1.strict).toBeUndefined();

    const f2 = result.find(
      t => t.type === 'function' && t.name === 'f2',
    ) as unknown as Record<string, unknown>;
    expect(f2).toBeDefined();
    expect(f2.strict).toBeUndefined();

    expect(result.find(t => t.type === 'web_search')).toEqual({
      type: 'web_search',
    });
  });

  it('does not modify mcp tools', () => {
    const tools: ResponsesApiTool[] = [
      {
        type: 'mcp',
        server_url: 'https://x',
