...
describe('normalizeLlamaStackEvent', () => {
  ...
  describe('reasoning/thinking', () => {
    ...
    it('normalizes reasoning summary delta the same as reasoning delta', () => {
      const raw = JSON.stringify({
        type: 'response.reasoning_summary_text.delta',
        delta: 'summarizing...',
      });
      expect(normalizeLlamaStackEvent(raw)).toEqual([
        { type: 'stream.reasoning.delta', delta: 'summarizing...' },
      ]);
    });

    it('normalizes reasoning summary done the same as reasoning done', () => {
      const raw = JSON.stringify({
        type: 'response.reasoning_summary_text.done',
        text: 'Summary conclusion.',
      });
      expect(normalizeLlamaStackEvent(raw)).toEqual([
        { type: 'stream.reasoning.done', text: 'Summary conclusion.' },
      ]);
    });

    it('handles reasoning_summary_part.added/done as no-ops', () => {
      expect(
        normalizeLlamaStackEvent(
          JSON.stringify({ type: 'response.reasoning_summary_part.added' }),
        ),
      ).toEqual([]);
      expect(
        normalizeLlamaStackEvent(
  ...
  describe('error sanitization', () => {
    it('sanitizes internal Kubernetes service URL', () => {
      const raw = JSON.stringify({
        type: 'response.mcp_call.failed',
        item_id: 'call-123',
        name: 'tool_name',
        server_label: 'my-mcp',
        error: 'connect ECONNREFUSED http://mcp-server.namespace.svc:8080',
      });
      const result = normalizeLlamaStackEvent(raw);
      expect(result).toHaveLength(1);
      expect(result[0]).toMatchObject({
      ...
      expect((result[0] as any).error).not.toContain(
        'mcp-server.namespace.svc',
      );
      expect((result[0] as any).error).not.toContain('8080');
    });

    it('passes through errors without URLs unchanged', () => {
      const raw = JSON.stringify({
        type: 'response.mcp_call.failed',
        item_id: 'call-123',
        name: 'tool_name',
    ...
    it('sanitizes raw URL errors without server_label', () => {
      const raw = JSON.stringify({
        type: 'response.mcp_call.failed',
        item_id: 'call-123',
        name: 'tool_name',
        error:
          'Failed to connect to https://internal.example.com:8080/mcp: connection refused',
      });
      const result = normalizeLlamaStackEvent(raw);
      expect(result).toHaveLength(1);
      expect((result[0] as any).error).not.toContain('internal.example.com');
      expect((result[0] as any).error).toContain('MCP server');
      expect((result[0] as any).error).toContain('unreachable');
    });

    it('does not leak URLs across consecutive sanitization calls', () => {
      const error1 = JSON.stringify({
        type: 'response.mcp_call.failed',
        item_id: 'call-1',
        name: 'tool1',
        error: 'https://first-server.example.com/mcp: connection refused',
      });
      const result1 = normalizeLlamaStackEvent(error1);
      expect((result1[0] as any).error).not.toContain(
        'first-server.example.com',
      );

      const error2 = JSON.stringify({
        type: 'response.mcp_call.failed',
        item_id: 'call-2',
        name: 'tool2',
        error: 'https://second-server.example.com/mcp: timeout',
      });
      const result2 = normalizeLlamaStackEvent(error2);
      expect((result2[0] as any).error).not.toContain(
        'second-server.example.com',
      );
      expect((result2[0] as any).error).toContain('timed out');
    });
  });
});
