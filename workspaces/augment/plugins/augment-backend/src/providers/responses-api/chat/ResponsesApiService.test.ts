...
describe('ResponsesApiService', () => {
  ...
  // -----------------------------------------------------------------------
  //  continueFunctionCallOutput
  // -----------------------------------------------------------------------

  describe('continueFunctionCallOutput', () => {
    it('sends function_call_output with previous_response_id', async () => {
      ...
      const result = await service.continueFunctionCallOutput({
        client: mockClient as unknown as ResponsesApiClient,
        model: 'test-model',
        callId: 'call-1',
        output: 'namespace list output',
    ...
    it('includes function_call input item when functionCall is provided', async () => {
      mockClient.request.mockResolvedValue({
        id: 'resp-2',
        output: [
          {
      ...
      await service.continueFunctionCallOutput({
        ...
        output: 'tool result',
        previousResponseId: 'resp-1',
        functionCall: { name: 'namespaces_list', arguments: '{"all":true}' },
      });
    ...
    it('falls back to conversation when previousResponseId is not provided', async () => {
      ...
      expect(body.conversation).toBe('conv_abc123');
      expect(body.previous_response_id).toBeUndefined();
    });

    it('omits conversation field when conversationId is not provided', async () => {
      mockClient.request.mockResolvedValue({
        id: 'resp-2',
        output: [],
      });
