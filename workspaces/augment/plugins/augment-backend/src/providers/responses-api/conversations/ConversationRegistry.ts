...
export class ConversationRegistry {
  ...
  private static readonly TABLE_NAME = 'augment_conversation_registry';
  ...
  /**
   * Remove all registry entries associated with a given conversationId.
   * Called when a conversation is deleted to prevent stale lookups.
   */
  async removeByConversationId(conversationId: string): Promise<void> {
    // Clean in-memory cache
    const keysToDelete: string[] = [];
    for (const [responseId, convId] of this.responseToConversation) {
      if (convId === conversationId) keysToDelete.push(responseId);
    }
    for (const key of keysToDelete) {
      this.responseToConversation.delete(key);
    }

    this.storedConversationIds.delete(conversationId);
    ...
    if (this.db) {
      try {
        ...
        const msg = toErrorMessage(error);
        this.logger.warn(
          `Failed to remove conversation registry entries for ${conversationId}: ${msg}`,
        );
      }
    }

    this.logger.debug(
      `Removed ${keysToDelete.length} registry entries for conversation ${conversationId}`,
    );
  }
