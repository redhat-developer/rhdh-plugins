...
/** Raw item shape from Llama Stack /v1/responses/{id}/input_items API */
...
export function mapRawItemsToConversationItems(
  ...
  return rawItems.map(item => {
    ...
    if (Array.isArray(item.summary)) {
      result.summary = (item.summary as Array<Record<string, unknown>>).map(
        s => ({
...
/**
 * Map raw API items from /v1/responses/{id}/input_items to InputItem[].
