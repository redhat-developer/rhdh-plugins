...
// Mock the ResponsesApiClient
jest.mock('../client/ResponsesApiClient');

describe('VectorStoreService', () => {
  let mockClient: jest.Mocked<ResponsesApiClient>;
  ...
  describe('ensureExists', () => {
    ...
    it('should use existing vectorStoreId if it validates successfully', async () => {
      ...
      expect(mockClient.request).toHaveBeenCalledWith(
        '/v1/vector_stores/vs_existing_123',
        { method: 'GET' },
      );
  ...
  describe('deleteVectorStore', () => {
    ...
    beforeEach(() => {
      service = new VectorStoreService(mockClient, baseConfig, mockLogger);
    });

    it('should delete all files then the store itself', async () => {
      mockClient.request
        .mockResolvedValueOnce({
          data: [
        ...
        .mockResolvedValueOnce({ success: true })
        .mockResolvedValueOnce({ success: true })
        .mockResolvedValueOnce({ id: 'vs_123', deleted: true });

      const result = await service.deleteVectorStore('vs_123');

      expect(result).toEqual({ success: true, filesDeleted: 2 });
      expect(mockClient.request).toHaveBeenCalledWith(
    ...
    it('should treat store-not-found as successful deletion', async () => {
      // Mock 1: fetchAllVectorStoreFiles → not found (caught, warned)
      // Mock 2: DELETE /v1/vector_stores/{id} → not found (treated as success)
      mockClient.request
        .mockRejectedValueOnce(
          new Error('Llama Stack API error: 400 - Vector_store not found'),
        )
        .mockRejectedValueOnce(
          new Error(
            "Llama Stack API error: 400 - Vector_store 'vs_gone' not found",
          ),
        );

      const result = await service.deleteVectorStore('vs_gone');

      expect(result).toEqual({ success: true, filesDeleted: 0 });
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining('not found on server'),
      );
    });
    ...
    it('should treat 404 as successful deletion', async () => {
      ...
      const result = await service.deleteVectorStore('vs_404');

      expect(result).toEqual({ success: true, filesDeleted: 0 });
    });

    it('should throw on real server errors', async () => {
      mockClient.request
        .mockRejectedValueOnce(new Error('connection refused'))
        .mockRejectedValueOnce(
          new Error('Llama Stack API error: 500 Internal Server Error'),
        );

      await expect(service.deleteVectorStore('vs_err')).rejects.toThrow(
        'Failed to delete vector store vs_err',
      );
