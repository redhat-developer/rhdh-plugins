...
function createMockClient(): jest.Mocked<Pick<ResponsesApiClient, 'request'>> {
  return {
    request: jest.fn(),
  };
}
...
function createDeps(
...
): DocumentListerDeps & { client: jest.Mocked<ResponsesApiClient> } {
  ...
  return {
    client: client as unknown as jest.Mocked<ResponsesApiClient>,
    config,
    logger: createMockLogger(),
    ...overrides,
  } as unknown as DocumentListerDeps & {
    client: jest.Mocked<ResponsesApiClient>;
  };
}
...
describe('DocumentLister', () => {
  describe('fetchAllVectorStoreFiles', () => {
    it('fetches files when response is object with data array', async () => {
      ...
      expect(deps.client.request).toHaveBeenCalledWith(
        '/v1/vector_stores/vs_123/files?limit=100',
        { method: 'GET' },
      );
    });
    ...
    it('uses custom vectorStoreId when provided', async () => {
      ...
      expect(deps.client.request).toHaveBeenCalledWith(
        '/v1/vector_stores/vs_custom/files?limit=100',
        { method: 'GET' },
      );
    });
    ...
    it('paginates when has_more is true', async () => {
      ...
      expect(deps.client.request).toHaveBeenNthCalledWith(
        2,
        '/v1/vector_stores/vs_123/files?limit=100&after=file_1',
        { method: 'GET' },
      );
    });
  ...
  describe('listDocuments', () => {
    it('returns documents from vector store files endpoint', async () => {
      ...
      expect(deps.client.request).toHaveBeenCalledWith('/v1/files/vsfile_1', {
        method: 'GET',
      });
    });
