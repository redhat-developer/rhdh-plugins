...
describe('CapabilityBuilders', () => {
  describe('buildConversationsCapability', () => {
    ...
    it('delete delegates to facade.deleteConversation', async () => {
      const cap = buildConversationsCapability(mockFacade);
      await cap.delete('r1');
      expect(mockFacade.deleteConversation).toHaveBeenCalledWith(
        'r1',
        undefined,
      );
    });

    it('deleteContainer delegates to facade.deleteConversationContainer', async () => {
      mockFacade.deleteConversationContainer = jest
        .fn()
        .mockResolvedValue(true);
      const cap = buildConversationsCapability(mockFacade);
      const result = await cap.deleteContainer!('conv-1');
      expect(mockFacade.deleteConversationContainer).toHaveBeenCalledWith(
        'conv-1',
      );
  ...
  describe('buildRagCapability', () => {
    ...
    it('generateAnswer is undefined when no inference accessor provided', () => {
      const cap = buildRagCapability(mockFacade);
      expect(cap.generateAnswer).toBeUndefined();
    });

    it('generateAnswer retrieves chunks and generates LLM answer', async () => {
      const searchResult = {
        query: 'What is RAG?',
        chunks: [
          {
      ...
      const cap = buildRagCapability(mockFacade, mockInference);
      const result = await cap.generateAnswer!('What is RAG?', 5, 'vs_1');
      ...
      expect(result.answer).toBe('RAG combines retrieval with generation.');
      expect(result.model).toBe('test-model');
      expect(result.chunks).toEqual(searchResult.chunks);
      expect(result.totalResults).toBe(1);
    });

    it('generateAnswer returns fallback when LLM produces no text', async () => {
      mockFacade.searchVectorStore.mockResolvedValue({
        query: 'test',
        chunks: [],
        vectorStoreId: 'vs_1',
        totalResults: 0,
      ...
      const mockInference = {
        getClient: () => ({
          request: jest.fn().mockResolvedValue({ output: [] }),
        }),
        getModel: () => 'test-model',
      };

      const cap = buildRagCapability(mockFacade, mockInference);
      const result = await cap.generateAnswer!('test');

      expect(result.answer).toBe(
        'The model returned no answer. Try rephrasing the query.',
      );
    });
  });
