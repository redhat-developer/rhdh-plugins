...
describe('ConversationFacade', () => {
  ...
  describe('deleteConversationContainer', () => {
    it('delegates to conversations service', async () => {
      mockConversations.deleteConversationContainer = jest
        .fn()
        .mockResolvedValue(true);

      const facade = createFacade();
      const result = await facade.deleteConversationContainer('conv-1');

      expect(mockContext.ensureInitialized).toHaveBeenCalled();
      expect(
        mockConversations.deleteConversationContainer,
  ...
  describe('continueAfterApproval', () => {
    ...
    it('routes to backend handler when pending approval exists and preserves store until handler completes', async () => {
      const { BackendApprovalStore } = jest.requireActual(
        './BackendApprovalStore',
      ) as {
        BackendApprovalStore: new () => import('./BackendApprovalStore').BackendApprovalStore;
      };
      ...
      const handlerResult: ApprovalResult = {
        content: 'Tool executed',
        responseId: 'r2',
        toolExecuted: true,
      };
      const handler = jest.fn().mockImplementation(async approval => {
        const stillInStore = store.get(approval.responseId, approval.callId);
        expect(stillInStore).toBeDefined();
        expect(stillInStore!.functionName).toBe('ocp-mcp__namespaces_list');
        return handlerResult;
      });

      const facade = createFacade();
      facade.setBackendApprovalHandler(store, handler);
      ...
      expect(store.get('r1', 'call-1')).toBeUndefined();
    });

    it('keeps pending approval when handler throws to allow retries', async () => {
      const { BackendApprovalStore } = jest.requireActual(
        './BackendApprovalStore',
      ) as {
        BackendApprovalStore: new () => import('./BackendApprovalStore').BackendApprovalStore;
      ...
      const handler = jest.fn().mockRejectedValue(new Error('LlamaStack down'));
      const facade = createFacade();
      facade.setBackendApprovalHandler(store, handler);

      await expect(
        facade.continueAfterApproval('r1', 'call-1', true),
      ).rejects.toThrow('LlamaStack down');

      expect(store.get('r1', 'call-1')).toBeDefined();
    });
  });
});
