...
import { ResponsesApiClient, ResponsesApiError } from './ResponsesApiClient';
import { createMockLogger } from '../../test-utils/mocks';

function createMockClient(): ResponsesApiClient & { request: jest.Mock } {
  const request = jest.fn();
  return { request } as unknown as ResponsesApiClient & { request: jest.Mock };
}
...
describe('ConversationService', () => {
  ...
  describe('deleteConversationContainer', () => {
    it('deletes the conversation container and returns true', async () => {
      const mockClient = createMockClient();
      mockClient.request.mockResolvedValue({});

      const service = createService({ client: mockClient });
      const result = await service.deleteConversationContainer('conv_123');
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining('Deleted conversation container conv_123'),
      );
    });

    it('returns false when the container deletion fails', async () => {
      const mockClient = createMockClient();
      mockClient.request.mockRejectedValue(new Error('Not found'));

      const service = createService({ client: mockClient });
      const result = await service.deleteConversationContainer('conv_bad');

      expect(result).toBe(false);
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          'Failed to delete conversation container conv_bad',
        ),
      );
    });
  });
  ...
  describe('dynamic client accessor', () => {
    it('picks up a new client when the accessor returns a different instance', async () => {
      ...
      let activeClient: ResponsesApiClient & { request: jest.Mock } = client1;
    ...
    it('registers approval response ID in registry for future lookups', async () => {
      ...
      const convId =
        await service.getConversationForResponse('resp-approval-new');
  ...
  describe('getConversationItems', () => {
    it('handles { data: [...] } response format', async () => {
      ...
      const service = createService({ client: mockClient });
      const result = await service.getConversationItems('conv-1');
    ...
    it('handles bare array response format', async () => {
      ...
      expect(result.items).toHaveLength(2);
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('bare array'),
      );
    });
    ...
    it('handles { items: [...] } response format', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining('"items" key'),
      );
    });

    it('returns empty array and warns for unexpected response shape', async () => {
      const mockClient = createMockClient();
      mockClient.request.mockResolvedValue('not-an-object');
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Unexpected conversation items response shape'),
      );
    });

    it('returns empty items on 404 instead of throwing', async () => {
      const mockClient = createMockClient();
      ...
      const service = createService({ client: mockClient });
      const result = await service.getConversationItems('conv-deleted');

      expect(result.items).toHaveLength(0);
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('not found (404)'),
      );
    });
    ...
    it('rethrows non-404 errors', async () => {
      const mockClient = createMockClient();
      mockClient.request.mockRejectedValue(
        new ResponsesApiError(500, 'Internal Server Error', '{}'),
      );

      const service = createService({ client: mockClient });

      await expect(service.getConversationItems('conv-1')).rejects.toThrow(
        'Responses API error',
      );
    });
  });
