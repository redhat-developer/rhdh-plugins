...
jest.mock('../../../services/utils/mcpClient', () => ({
  listMcpServerTools: jest.fn(),
}));

import { listMcpServerTools } from '../../../services/utils/mcpClient';

const mockListMcpServerTools = listMcpServerTools as jest.MockedFunction<
  typeof listMcpServerTools
>;
...
describe('StatusService', () => {
  ...
  // ===========================================================================
  // MCP server health (using SDK mock)
  // ===========================================================================
  ...
  it('reports MCP server as connected when health check succeeds', async () => {
    ...
    expect(mockListMcpServerTools).toHaveBeenCalledWith(
      'http://mcp.test/sse',
      expect.objectContaining({ clientName: 'augment-status' }),
    );
  });
  ...
  it('reports MCP server as disconnected when health check fails', async () => {
    ...
    mockListMcpServerTools.mockRejectedValue(new Error('Connection refused'));

    const status = await service.getStatus(baseDeps({ mcpServers }));

    expect(status.mcpServers[0].connected).toBe(false);
    expect(status.mcpServers[0].error).toBe('Connection refused');
    expect(status.capabilities?.mcpTools.available).toBe(false);
  ...
  it('reports MCP server error on network failure', async () => {
    ...
    mockListMcpServerTools.mockRejectedValue(new Error('ECONNREFUSED'));
  ...
  it('includes MCP auth headers when mcpAuth is provided', async () => {
    ...
    await service.getStatus(
      baseDeps({
        mcpServers,
        mcpAuth: mockMcpAuth as unknown as McpAuthService,
      }),
    );
