 ...
 *
 * Extracted from ResponsesApiCoordinator so config-resolution logic
...
export class ConfigResolutionService {
  ...
  private postToolInstructions?: string;
  ...
  private lastResolvedConfig: EffectiveConfig | null = null;
  ...
  setPostToolInstructions(instructions: string | undefined): void {
    this.postToolInstructions = instructions;
  }
  ...
  /**
   * Return the most recently resolved config without triggering a
   * new resolution. Returns null if `resolve()` has never been called.
   */
  ...
  buildYamlFallback(): EffectiveConfig {
    ...
    return {
      ...
      postToolInstructions: this.postToolInstructions,
    };
  }

  /**
   * Return the stored LlamaStackConfig, so callers can check if
   * initialization has happened.
   */
  isConfigured(): boolean {
    return this.llamaStackConfig !== null;
  }
}
