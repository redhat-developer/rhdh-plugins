  ...
  private parseAgentConfigs():
    ...
    for (const [key, value] of Object.entries(agentsRaw)) {
      if (!value || typeof value !== 'object') {
        this.logger.warn(
          `[MultiAgent] Skipping invalid agent config: "${key}"`,
        );
        continue;
      }
      ...
      const agent: AgentConfig = {
        ...
        handoffs: Array.isArray(raw.handoffs)
          ? (raw.handoffs as string[])
          : undefined,
        asTools: Array.isArray(raw.asTools)
          ? (raw.asTools as string[])
          : undefined,
        ...
        toolUseBehavior: raw.toolUseBehavior as
          ...
          | undefined,
        outputSchema: raw.outputSchema as
      ...
      if (raw.mcpServers && !Array.isArray(raw.mcpServers)) {
        this.logger.warn(
          `[MultiAgent] Agent "${key}": mcpServers should be an array, got ${typeof raw.mcpServers}. Ignoring.`,
        );
      }
      if (raw.handoffs && !Array.isArray(raw.handoffs)) {
        this.logger.warn(
          `[MultiAgent] Agent "${key}": handoffs should be an array, got ${typeof raw.handoffs}. Ignoring.`,
        );
      }
      if (raw.asTools && !Array.isArray(raw.asTools)) {
        this.logger.warn(
          `[MultiAgent] Agent "${key}": asTools should be an array, got ${typeof raw.asTools}. Ignoring.`,
        );
      }
