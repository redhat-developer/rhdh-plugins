...
describe('ConfigLoader', () => {
  describe('validateRequiredConfig', () => {
    ...
    it('throws ConfigValidationError when llamaStack section is missing', () => {
      const { loader } = createConfigLoader({ augment: {} });
    ...
    it('throws when baseUrl is missing', () => {
      const { loader } = createConfigLoader({
        augment: { llamaStack: { model: 'test-model' } },
      });
      expect(() => loader.validateRequiredConfig()).toThrow(
        ConfigValidationError,
      );
  ...
  describe('loadLlamaStackConfig', () => {
    ...
    it('loads minimal config with defaults', () => {
      ...
      expect(result.vectorStoreName).toBe('augment-knowledge-base');
    ...
    it('loads vectorStoreIds array', () => {
      const { loader } = createConfigLoader({
        augment: {
          llamaStack: {
            ...minimalConfig.augment.llamaStack,
  ...
  describe('loadSystemPrompt', () => {
    it('returns configured prompt', () => {
      const { loader } = createConfigLoader({
        augment: { systemPrompt: 'You are a helpful assistant' },
      });
  ...
  describe('loadPromptGroups', () => {
    it('returns empty array when no prompt groups configured', () => {
      const { loader } = createConfigLoader({});
      expect(loader.loadPromptGroups()).toEqual([]);
    });

    it('parses prompt groups with cards', () => {
      const { loader } = createConfigLoader({
        augment: {
          promptGroups: [
            {
      ...
      const result = loader.loadPromptGroups();
      expect(result).toHaveLength(1);
      expect(result[0].cards).toHaveLength(1);
    });

    it('sorts prompt groups by order', () => {
      const { loader } = createConfigLoader({
        augment: {
          promptGroups: [
  ...
  describe('loadToolExecutionMode', () => {
    it('returns backend when not configured (default)', () => {
      const { loader } = createConfigLoader({
        augment: {
          llamaStack: { baseUrl: 'http://localhost', model: 'test' },
        },
      });
      expect(loader.loadToolExecutionMode()).toBe('backend');
    });

    it('returns backend when configured as backend', () => {
      const { loader } = createConfigLoader({
        augment: {
          toolExecutionMode: 'backend',
          llamaStack: { baseUrl: 'http://localhost', model: 'test' },
        },
      });
      expect(loader.loadToolExecutionMode()).toBe('backend');
    ...
    it('returns backend case-insensitively', () => {
      const { loader } = createConfigLoader({
        augment: {
          toolExecutionMode: 'Backend',
          llamaStack: { baseUrl: 'http://localhost', model: 'test' },
    ...
    it('returns direct for explicit direct value', () => {
      const { loader } = createConfigLoader({
        augment: {
      ...
      expect(loader.loadToolExecutionMode()).toBe('direct');
    });

    it('warns and falls back to direct for invalid values', () => {
      const { loader, logger } = createConfigLoader({
        augment: {
          toolExecutionMode: 'proxy',
          llamaStack: { baseUrl: 'http://localhost', model: 'test' },
        },
      });
      expect(loader.loadToolExecutionMode()).toBe('direct');
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining("Invalid toolExecutionMode 'proxy'"),
      );
    });
  });
});
