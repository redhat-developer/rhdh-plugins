...
describe('ClientManager', () => {
  ...
  describe('getClient()', () => {
    ...
    it('creates a new client when baseUrl changes', () => {
      const client1 = manager.getClient(
        createConfig({ baseUrl: 'https://host-a:8321' }),
      );
      const client2 = manager.getClient(
        createConfig({ baseUrl: 'https://host-b:8321' }),
      );
  ...
  describe('getExistingClient()', () => {
    ...
    it('returns the latest client after identity change', () => {
      manager.getClient(createConfig({ baseUrl: 'https://host-a:8321' }));
  ...
  describe('client configuration', () => {
    it('created client has the correct baseUrl', () => {
      const config = createConfig({ baseUrl: 'https://my-llama:9999' });
      const client = manager.getClient(config);

      expect(client.getConfig().baseUrl).toBe('https://my-llama:9999');
    });

    it('re-created client uses the new config', () => {
      manager.getClient(createConfig({ baseUrl: 'https://old:8321' }));
      const newClient = manager.getClient(
        createConfig({ baseUrl: 'https://new:8321', token: 'fresh-token' }),
      );

      expect(newClient.getConfig().baseUrl).toBe('https://new:8321');
