...
function createMockClientManager(): jest.Mocked<ClientManager> {
  ...
  return {
    ...
    probeServerVersion: jest.fn().mockResolvedValue(undefined),
...
describe('initializeOrchestrator', () => {
  ...
  describe('backendToolExecutor singleton', () => {
    it('creates BackendToolExecutor when toolExecutionMode is backend', async () => {
      const deps = createBaseDeps();
      (deps.configLoader as any).loadToolExecutionMode = jest
        .fn()
        .mockReturnValue('backend');

      const result = await initializeOrchestrator(deps);

      expect(result.backendToolExecutor).toBeDefined();
      expect(result.backendToolExecutor).not.toBeNull();
    });

    it('does not create BackendToolExecutor when toolExecutionMode is direct', async () => {
      const deps = createBaseDeps();

      const result = await initializeOrchestrator(deps);

      expect(result.backendToolExecutor).toBeNull();
    });
  });
