...
function createMockClient(): jest.Mocked<ResponsesApiClient> {
  return {
    request: jest.fn(),
...
function makeConfig(overrides: Record<string, unknown> = {}) {
  ...
  return {
    getOptionalConfig: jest.fn((key: string) => {
      if (key === 'augment.llamaStack') return llamaStackConfig;
      if (key === 'augment.evaluation') return evalConfig;
      return undefined;
    }),
  ...
  describe('initialize', () => {
    ...
    it('warns when enabled but no client accessor provided', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('ResponsesApiClient accessor'),
      );
    });
  ...
  describe('scoreResponse', () => {
    ...
    it('sends correct Llama Stack field names (input_query, generated_answer)', async () => {
      ...
      const scoreCall = mockClient.request.mock.calls.find(
        (c: unknown[]) => c[0] === '/v1/scoring/score',
      );
