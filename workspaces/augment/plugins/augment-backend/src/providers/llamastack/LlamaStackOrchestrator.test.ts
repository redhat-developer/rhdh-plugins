...
jest.mock('./ConfigLoader', () => {
  const mockInstance = {
    ...
    loadSecurityConfig: jest.fn().mockReturnValue({ mode: 'plugin-only' }),
    loadSystemPrompt: jest.fn().mockReturnValue(''),
    loadPostToolInstructions: jest.fn().mockReturnValue(undefined),
  };
  return {
    ConfigLoader: jest.fn().mockImplementation(() => mockInstance),
  };
});
