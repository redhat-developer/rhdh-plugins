...
jest.mock('./ResponsesApiCoordinator', () => ({
  ResponsesApiCoordinator: jest.fn().mockImplementation(() => mockOrchestrator),
}));
...
describe('ResponsesApiProvider', () => {
  describe('constructor', () => {
    it('creates orchestrator, safety, and evaluation services', () => {
      const { ResponsesApiCoordinator } = require('./ResponsesApiCoordinator');
      ...
      expect(ResponsesApiCoordinator).toHaveBeenCalledWith(
  ...
  describe('getEffectiveConfig', () => {
    ...
    it('returns config when resolver is set', async () => {
      const resolvedConfig = { model: 'test-model', baseUrl: 'https://test' };
  ...
  describe('chatStream', () => {
    ...
    it('passes through Runner-emitted stream.tool.approval events', async () => {
      const request = { messages: [], model: 'test' };
      const onEvent = jest.fn();
      const logger = createMockLogger();
      ...
      mockOrchestrator.chatStream.mockImplementation(
        async (
          ...
          // Runner-emitted event (already normalized, not a LlamaStack event)
          rawCallback(
            JSON.stringify({
              type: 'stream.tool.approval',
              callId: 'call-backend-1',
              name: 'list_pods',
              serverLabel: 'ocp-mcp',
      ...
      const approvalEvent = onEvent.mock.calls.find(
        (c: [NormalizedStreamEvent]) =>
          c[0].type === 'stream.tool.approval' &&
          (c[0] as unknown as Record<string, unknown>).callId ===
            'call-backend-1',
      );
    ...
    it('passes through Runner-emitted stream.backend_tool.executing events', async () => {
      const request = { messages: [], model: 'test' };
      const onEvent = jest.fn();
      const provider = createProvider();
        ...
        async (
          ...
          rawCallback(
            JSON.stringify({
              ...
              toolCount: 2,
              tools: ['ocp__list_pods', 'sn__find_tickets'],
            }),
          );
        },
      );
      ...
      const execEvent = onEvent.mock.calls.find(
        (c: [NormalizedStreamEvent]) =>
          c[0].type === 'stream.backend_tool.executing',
      );
      expect(execEvent).toBeDefined();
      expect(execEvent[0]).toMatchObject({
        type: 'stream.backend_tool.executing',
        toolCount: 2,
      });
    });

    it('passes through Runner-emitted stream.error events', async () => {
      const request = { messages: [], model: 'test' };
      const onEvent = jest.fn();
      const provider = createProvider();
      ...
      const errorEvent = onEvent.mock.calls.find(
        (c: [NormalizedStreamEvent]) => c[0].type === 'stream.error',
      );
