  ...
  describe('aggregateStatus', () => {
    ...
    it('uses resolved model/baseUrl over llamaStackConfig values', async () => {
      const deps = createBaseDeps({
        ...
        resolved: {
          baseUrl: 'https://admin-override:8321',
          model: 'admin-model',
      ...
      expect(call.config.baseUrl).toBe('https://admin-override:8321');
    ...
    it('passes yamlServerIds derived from yamlMcpServers', async () => {
      const yamlServers = [
        { id: 'server-1', name: 'S1', url: 'https://s1' },
        { id: 'server-2', name: 'S2', url: 'https://s2' },
