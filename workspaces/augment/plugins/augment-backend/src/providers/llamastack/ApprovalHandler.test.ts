...
const BASE_OPTS = {
  ...
  tools: [{ type: 'mcp', server_url: 'https://mcp' }],
};
...
describe('buildApprovalRequest', () => {
  it('sends native mcp_approval_response with approve: true when approved', () => {
    ...
    expect(result.tools).toEqual([
      { type: 'mcp', server_url: 'https://mcp', require_approval: 'never' },
    ]);
  ...
  it('always uses previous_response_id', () => {
  ...
  it('includes tools array for both approval and rejection', () => {
    ...
    expect(approved.tools).toEqual(expectedTools);
    expect(rejected.tools).toEqual(expectedTools);
  });
});
