...
describe('AdminConfigService', () => {
  ...
  describe('before initialization', () => {
    it('throws on get when not initialized', async () => {
      ...
      await expect(service.get('promptGroups')).rejects.toThrow(
    ...
    it('throws on set when not initialized', async () => {
      ...
      const service = new AdminConfigService(mockDb, mockLogger);
      await expect(
        service.set('promptGroups', [], 'user:default/test'),
    ...
    it('throws on delete when not initialized', async () => {
      const mockDb = { getClient: jest.fn() };
      const service = new AdminConfigService(mockDb, mockLogger);
      await expect(service.delete('promptGroups')).rejects.toThrow(
        'AdminConfigService not initialized',
      );
    });
  ...
  describe('validateKey', () => {
    it('accepts valid keys', () => {
      expect(AdminConfigService.validateKey('promptGroups')).toBe(
        'promptGroups',
      );
...
describe('AdminConfigService (with database)', () => {
  ...
  describe('get', () => {
    it('returns undefined for a key that has not been set', async () => {
      const result = await service.get('promptGroups');
      expect(result).toBeUndefined();
    });

    it('returns the value after it has been set', async () => {
      const lanes = [{ id: 'lane-1', title: 'Test Lane', cards: [] }];
      await service.set('promptGroups', lanes, 'user:default/admin');
      const result = await service.get('promptGroups');
    ...
    it('rejects invalid keys', async () => {
      await expect(
        service.get(
          'invalidKey' as import('@red-hat-developer-hub/backstage-plugin-augment-common').AdminConfigKey,
        ),
  ...
  describe('set', () => {
    ...
    it('handles complex JSON objects round-trip', async () => {
      ...
      await service.set('promptGroups', complex, 'user:default/admin');
      const result = await service.get('promptGroups');
    ...
    it('rejects invalid keys', async () => {
      await expect(
        service.set(
          'badKey' as import('@red-hat-developer-hub/backstage-plugin-augment-common').AdminConfigKey,
          'value',
          'user:default/admin',
        ),
      ).rejects.toThrow(InputError);
  ...
  describe('delete', () => {
    it('returns true when deleting an existing entry', async () => {
      await service.set('promptGroups', [], 'user:default/admin');
      const deleted = await service.delete('promptGroups');
  ...
  describe('listAll', () => {
    ...
    it('returns all entries sorted by key', async () => {
      ...
      expect(entries[1].configKey).toBe('promptGroups');
    ...
    it('reflects deletions', async () => {
      await service.set('promptGroups', [], 'user:default/admin');
      await service.set('branding', {}, 'user:default/admin');
      await service.delete('promptGroups');
  ...
  describe('multiple keys isolation', () => {
    it('setting one key does not affect another', async () => {
      await service.set('promptGroups', [{ id: 'l1' }], 'user:default/admin');
      await service.set('branding', { appName: 'Test' }, 'user:default/admin');

      expect(await service.get('promptGroups')).toEqual([{ id: 'l1' }]);
      expect(await service.get('branding')).toEqual({ appName: 'Test' });

      await service.set('promptGroups', [{ id: 'l2' }], 'user:default/admin');
      expect(await service.get('branding')).toEqual({ appName: 'Test' });
    });
  });
});
