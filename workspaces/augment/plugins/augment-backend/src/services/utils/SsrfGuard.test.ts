  ...
  describe('isPrivateUrl', () => {
    describe('public URLs', () => {
      ...
      it('returns null for other public domains', () => {
        ...
        expect(isPrivateUrl('https://example.org/path')).toBeNull();
      });
    });

    describe('localhost', () => {
      it('returns reason for https://localhost:8080', () => {
        expect(isPrivateUrl('https://localhost:8080')).toBe('localhost');
      });

      it('returns reason for https://127.0.0.1:3000', () => {
        expect(isPrivateUrl('https://127.0.0.1:3000')).toBe('loopback');
      });
    ...
    describe('private IPs (RFC-1918)', () => {
      it('returns reason for 10.0.0.1', () => {
        expect(isPrivateUrl('https://10.0.0.1')).toBe('RFC-1918');
      });

      it('returns reason for 172.16.0.1', () => {
        expect(isPrivateUrl('https://172.16.0.1:8080')).toBe('RFC-1918');
      });
      ...
      it('returns reason for 192.168.1.1', () => {
        expect(isPrivateUrl('https://192.168.1.1')).toBe('RFC-1918');
      });
    ...
    describe('link-local (169.254.x.x)', () => {
      it('returns reason for 169.254.1.1', () => {
        expect(isPrivateUrl('https://169.254.1.1')).toBe('link-local');
      });
    ...
    describe('cloud metadata endpoint (169.254.169.254)', () => {
      it('returns reason for 169.254.169.254 (matches link-local pattern first)', () => {
        ...
        expect(isPrivateUrl('https://169.254.169.254/latest/meta-data/')).toBe(
          'link-local',
        );
      });
    });
    ...
    describe('IPv6 loopback', () => {
      it('returns reason for ::1', () => {
        expect(isPrivateUrl('https://[::1]:3000')).toBe('IPv6 loopback');
      });

      it('returns reason for ::1 without brackets in hostname extraction', () => {
        expect(isPrivateUrl('https://[::1]/')).toBe('IPv6 loopback');
      });
    });
    ...
    describe('blocked hostnames', () => {
      it('returns reason for metadata.google.internal', () => {
        expect(
          isPrivateUrl('https://metadata.google.internal/computeMetadata/v1/'),
        ).toBe('blocked host: metadata.google.internal');
      });

      it('returns reason for metadata.goog', () => {
        expect(isPrivateUrl('https://metadata.goog/')).toBe(
    ...
    describe('malformed URLs', () => {
      ...
      it('returns reason for URL with invalid characters', () => {
        expect(isPrivateUrl('https://[invalid')).toBe('invalid URL');
      });
    ...
    describe('non-HTTP protocols', () => {
      it('blocks ftp:// protocol', () => {
        expect(isPrivateUrl('ftp://example.com')).toBe(
          'blocked protocol: ftp:',
        );
      });

      it('blocks file:// protocol', () => {
        expect(isPrivateUrl('file:///etc/passwd')).toBe(
          'blocked protocol: file:',
        );
      });

      it('blocks data: protocol', () => {
        expect(isPrivateUrl('data:text/html,<h1>test</h1>')).toBe(
    ...
    describe('IPv4-mapped IPv6', () => {
      it('returns reason for ::ffff:127.0.0.1', () => {
        expect(isPrivateUrl('https://[::ffff:127.0.0.1]')).toBe(
          'IPv4-mapped IPv6',
        );
      });

      it('returns reason for ::ffff:10.0.0.1', () => {
        expect(isPrivateUrl('https://[::ffff:10.0.0.1]')).toBe(
          'IPv4-mapped IPv6',
        );
      });

      it('returns reason for ::ffff:192.168.1.1', () => {
        expect(isPrivateUrl('https://[::ffff:192.168.1.1]')).toBe(
          'IPv4-mapped IPv6',
        );
      });
    });
    ...
    describe('unspecified and IPv6 ULA/link-local', () => {
      it('returns reason for 0.0.0.0', () => {
        expect(isPrivateUrl('https://0.0.0.0')).toBe('unspecified');
      });

      it('returns reason for IPv6 ULA (fc00::)', () => {
        expect(isPrivateUrl('https://[fc00::1]')).toBe('IPv6 ULA');
      });

      it('returns reason for IPv6 link-local (fe80::)', () => {
        expect(isPrivateUrl('https://[fe80::1]')).toBe('IPv6 link-local');
      });
    });
  });
});
