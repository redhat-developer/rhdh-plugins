...
function createMockConfigLoader(
  ...
  return {
    ...
    loadPromptGroups: jest.fn(),
    loadAgentConfigs: jest.fn().mockReturnValue(undefined),
    loadGuardrailShieldIds: jest.fn().mockReturnValue(undefined),
...
describe('RuntimeConfigResolver', () => {
  ...
  describe('production parameters (guardrails, maxToolCalls, temperature, etc.)', () => {
    it('passes maxToolCalls from YAML baseline', async () => {
      const logger = createMockLogger();
      const configLoader = createMockConfigLoader({ maxToolCalls: 25 });
      const adminConfig = createMockAdminConfig();
    ...
    it('passes maxOutputTokens from YAML baseline', async () => {
      const logger = createMockLogger();
      const configLoader = createMockConfigLoader({ maxOutputTokens: 4096 });
      const adminConfig = createMockAdminConfig();
    ...
    it('passes temperature from YAML baseline', async () => {
      const logger = createMockLogger();
      const configLoader = createMockConfigLoader({ temperature: 0.7 });
      const adminConfig = createMockAdminConfig();
    ...
    it('passes safetyIdentifier from YAML baseline', async () => {
      const logger = createMockLogger();
      const configLoader = createMockConfigLoader({
        safetyIdentifier: 'cts-platform',
      });
      const adminConfig = createMockAdminConfig();
      ...
      const config = await resolver.resolve();
      expect(config.safetyIdentifier).toBe('cts-platform');
    });

    it('loads guardrails from safety shield IDs', async () => {
      const logger = createMockLogger();
      const configLoader = createMockConfigLoader();
      (configLoader.loadGuardrailShieldIds as jest.Mock).mockReturnValue([
        'content_safety',
        'prompt_injection',
      ]);
      const adminConfig = createMockAdminConfig();
      ...
      const config = await resolver.resolve();
      expect(config.guardrails).toEqual(['content_safety', 'prompt_injection']);
    });

    it('omits guardrails when no shields are configured', async () => {
      const logger = createMockLogger();
      const configLoader = createMockConfigLoader();
      (configLoader.loadGuardrailShieldIds as jest.Mock).mockReturnValue(
        undefined,
    ...
    it('production params default to undefined when not set', async () => {
      const logger = createMockLogger();
      const configLoader = createMockConfigLoader();
      const adminConfig = createMockAdminConfig();
      ...
      expect(config.safetyIdentifier).toBeUndefined();
      expect(config.guardrails).toBeUndefined();
    });
  });
});
