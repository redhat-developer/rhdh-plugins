...
describe('ChatSessionService (with database)', () => {
  ...
  describe('listSessions', () => {
    ...
    it('respects offset param for pagination', async () => {
      const created: Awaited<ReturnType<typeof service.createSession>>[] = [];
      for (let i = 0; i < 5; i++) {
        await new Promise(r => setTimeout(r, 5));
        created.push(
          await service.createSession('user:default/alice', `Session ${i + 1}`),
        );
      }
      // newest first: [5, 4, 3, 2, 1]
      const page1 = await service.listSessions('user:default/alice', 2, 0);
      expect(page1).toHaveLength(2);
      expect(page1[0].id).toBe(created[4].id);
      expect(page1[1].id).toBe(created[3].id);

      const page2 = await service.listSessions('user:default/alice', 2, 2);
      expect(page2).toHaveLength(2);
      expect(page2[0].id).toBe(created[2].id);
      expect(page2[1].id).toBe(created[1].id);

      const page3 = await service.listSessions('user:default/alice', 2, 4);
      expect(page3).toHaveLength(1);
      expect(page3[0].id).toBe(created[0].id);
    });

    it('returns empty when offset exceeds total', async () => {
      await service.createSession('user:default/alice', 'Only one');
      const sessions = await service.listSessions('user:default/alice', 10, 50);
      expect(sessions).toEqual([]);
    });

    it('treats negative offset as 0', async () => {
      await service.createSession('user:default/alice', 'A');
      const sessions = await service.listSessions('user:default/alice', 10, -5);
