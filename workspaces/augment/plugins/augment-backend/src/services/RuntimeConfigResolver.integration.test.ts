...
describe('Config Pipeline Integration: UI → DB → Resolver → EffectiveConfig', () => {
  ...
  afterEach(async () => {
    await db('augment_admin_config').del();
  ...
  // -------------------------------------------------------------------------
  // Prompt groups: not part of EffectiveConfig — consumed via direct DB read
  // in the /prompt-groups route handler. Verified in adminRoutes.test.ts.
  // -------------------------------------------------------------------------
