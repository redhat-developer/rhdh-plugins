...
export class DocumentIngestionService {
  ...
  /**
   * Fetch a URL with manual redirect handling to re-validate each
   * redirect target against the SSRF guard (prevents open-redirect SSRF).
   */
  private async fetchUrl(
    url: string,
    headers: Record<string, string> = {},
  ): Promise<import('./utils/http').TlsResponse> {
    let currentUrl = url;

    for (let i = 0; i < DocumentIngestionService.MAX_REDIRECTS; i++) {
      const response = await fetchWithTlsControl(currentUrl, {
        headers,
      ...
      if (response.status >= 300 && response.status < 400) {
        ...
        const blocked = await isPrivateUrlWithDns(redirectUrl);
        if (blocked) {
          throw new Error(
            `Redirect to ${redirectUrl} blocked by SSRF protection (${blocked})`,
          );
        }
    ...
    throw new Error(
      `Too many redirects (max ${DocumentIngestionService.MAX_REDIRECTS})`,
    );
  }
  ...
  private async fetchFromGitHub(
    ...
    try {
      ...
      const headers: Record<string, string> = {
        ...
        'User-Agent': 'augment-backstage-plugin',
      };
