...
export interface Config {
  /**
   * Configuration for Augment plugin
   *
   * ## Security Modes
   *
   * Augment supports 3 security modes, configurable via `security.mode`:
   *
   * ### Mode 1: 'none' - No Access Control
   * - Anyone can access Augment
   ...
   * - Users must be in a Keycloak group to access Augment
  ...
  augment?: {
     ...
     * Can also be toggled at runtime via: window.enableAugmentDebug()
     * @visibility frontend
     */
    debug?: boolean;
    ...
    security?: {
      /**
       * Security mode:
       * - 'none': No access control (anyone can use Augment)
       ...
       * (permission.enabled: false). When permissions ARE enabled (e.g., with RBAC configured),
    ...
    /**
     * Tool execution mode for MCP servers
     * - 'direct': LlamaStack calls MCP servers directly (default)
     * - 'backend': Backstage backend executes MCP tools on behalf of LlamaStack
     *   (required for disconnected/air-gapped clusters where LlamaStack cannot
     *    reach MCP servers)
     * @visibility backend
     */
    ...
    /**
     * Inference server configuration for RAG and chat
     */
    llamaStack: {
      /**
    ...
    mcpServers?: Array<{
       ...
       * If configured, Augment will obtain tokens from the OAuth provider
    ...
    branding?: {
      ...
      /**
       * Custom bot avatar URL (http/https only)
       * Overrides the default sparkle icon with a custom image
       * @visibility frontend
       */
      botAvatarUrl?: string;
       ...
       * Presets provide pre-configured color palettes.
       *
       * Available presets:
       * - 'default': Steel blue professional theme
       * - 'enterprise': Professional navy/blue theme
       *
       * Default: 'default'
       * @visibility frontend
       */
     ...
     * Uses safety shields for input/output validation
     * @visibility backend
     */
    safety?: {
      /**
      ...
      /**
       * Shield IDs to register with the inference server
       * @visibility backend
       */
     ...
     * Uses provider Scoring API to assess response quality
     * @visibility backend
     */
    evaluation?: {
      /**
       ...
       * Available functions depend on the inference server configuration
    ...
    /**
     * Override auto-detected Llama Stack server capabilities.
     * Useful when the plugin cannot detect the server version or when
     * running a custom/forked build of Llama Stack.
     *
     * @example
     * ```yaml
     * augment:
     *   serverCapabilities:
     *     strictField: false   # Llama Stack < 0.6.0
     *     maxOutputTokens: false
     * ```
     * @visibility backend
     */
    serverCapabilities?: {
      /** Can send `{ type: 'function' }` tools (default: true) */
      functionTools?: boolean;
      /** Can include the `strict` field on function tools (default: true for >= 0.6.0) */
      strictField?: boolean;
      /** Can send `max_output_tokens` in the request (default: true for >= 0.6.0) */
      maxOutputTokens?: boolean;
      /** Can send `{ type: 'mcp' }` tools (default: true) */
      mcpTools?: boolean;
      /** Supports `parallel_tool_calls` parameter (default: true) */
      parallelToolCalls?: boolean;
    };
    ...
    /**
     * Prompt groups - grouped prompt cards for the welcome screen
     * @visibility frontend
     */
    promptGroups?: Array<{
      /**
