# Keycloak Configuration for Augment

This guide provides step-by-step instructions to configure Keycloak for Augment authentication and authorization.
...
Create groups to control access to Augment.

### 4.1 Create augment-users Group

1. Go to **Groups** → **Create group**
2. Name: `augment-users`
3. Click **Create**
...
- `augment-admins` - Full access
- `augment-readonly` - Read-only access
...
> **Note**: Do NOT add `user1` to `augment-users` group. This user will be denied access.
...
Get a token for `user1` and verify it does NOT have the `augment-users` group:
...
| Group         | `augment-users`       | Access to Augment             |
