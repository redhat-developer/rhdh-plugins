...
## Augment Backend Configuration

Configure Augment to use the secured MCP server:
...
augment:
  ...
  security:
    ...
    # mode: 'full'       # Augment sends OAuth tokens to MCP server
...
| `full`        | OAuth Bearer token | Augment fetches token from Keycloak     |
...
# You can see this in Augment's chat interface
```
