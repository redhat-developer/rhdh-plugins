#!/bin/bash
#
# Augment - Full System Validation
...
print_header() {
    echo ""
    echo -e "${BOLD}╔════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}║  Augment - Full System Validation           ║${NC}"
    echo -e "${BOLD}╚════════════════════════════════════════════════════════════╝${NC}"
    echo ""
...
test_keycloak() {
    ...
    if [ -n "$BACKSTAGE_CLIENT_SECRET" ]; then
        ...
        if [ -n "$TOKEN" ] && [ "$TOKEN" != "null" ]; then
            ...
            # Check groups
            HAS_GROUP=$(echo "$CLAIMS" | jq -r '.groups | if . == null then false else contains(["augment-users"]) end' 2>/dev/null)
            if [ "$HAS_GROUP" = "true" ]; then
                log_pass "Token contains 'augment-users' group"
            else
                log_warn "Token missing 'augment-users' group"
            fi
...
test_backstage() {
    ...
    # Test Augment health
    HTTP=$(curl -sk -o /dev/null -w "%{http_code}" "${BACKSTAGE_URL}/api/augment/health" 2>/dev/null)
    if [ "$HTTP" = "200" ]; then
        log_pass "Augment plugin health check OK"
    else
        log_fail "Augment health returned HTTP $HTTP"
    fi
    ...
    if [ -f /tmp/backstage_cookies.txt ]; then
        ...
        if [ -n "$BACKSTAGE_TOKEN" ]; then
            ...
            PERM=$(curl -sk -X POST "${BACKSTAGE_URL}/api/permission/authorize" \
                ...
                -d '{"items":[{"id":"test","permission":{"type":"basic","name":"augment.access","attributes":{"action":"read"}}}]}' 2>/dev/null)
            ...
            if [ "$HTTP" = "200" ]; then
                log_pass "Augment API access: OK"
                COUNT=$(jq -r '.workflows | length' /tmp/workflows.json 2>/dev/null)
                log_info "Workflows available: $COUNT"
            elif [ "$HTTP" = "403" ]; then
                log_fail "Augment API: Access Denied (403)"
            else
                log_warn "Augment API returned HTTP $HTTP"
            fi
        else
...
print_summary() {
    ...
    if [ $FAIL -eq 0 ]; then
        ...
        echo ""
        echo "Augment is properly configured with:"
        echo "  • Keycloak for authentication"
        echo "  • Group-based access control (augment-users)"
...
print_architecture() {
    ...
    echo "  g, group:default/augment-users, role:default/augment-user"
    echo ""
    echo "  # Permission grant"
    echo "  p, role:default/augment-user, augment.access, read, allow"
    echo ""
    ...
    echo "  • Augment Backend: Checks augment.access permission"
