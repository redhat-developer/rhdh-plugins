#!/bin/bash
#
# Augment - Backstage Configuration Generator
#
# This script generates Backstage configuration for:
# - OIDC authentication with Keycloak
# - RBAC policies for Augment access control
...
# Backstage OIDC Authentication Configuration for Augment
# Generated on $(date)
#
...
# Backstage RBAC Configuration for Augment
# Generated on $(date)
#
# This configures permission-based access control for Augment.
# Users must be in the 'augment-users' Keycloak group to access the plugin.
#
# Tested with:
# - @backstage/plugin-permission-backend 0.5.x+
...
permission:
  ...
  rbac:
    ...
    policies:
      ...
      - p, role:default/augment-user, catalog.entity.read, read, allow
      ...
      # - p, role:default/augment-admin, catalog.entity.read, read, allow
...
# RHDH Team (also gets Augment access)
g, group:default/rhdh-team, role:default/augment-user

# Admins
# g, group:default/augment-admins, role:default/augment-admin
# g, role:default/augment-admin, role:default/augment-user

# ============================================
# Permission Grants
# ============================================

# Augment Plugin Access
p, role:default/augment-user, augment.access, read, allow
...
# p, user:default/blocked-user, augment.access, read, deny
EOF
...
generate_catalog_entities() {
    ...
    cat > "${OUTPUT_DIR}/augment-entities.yaml" << EOF
# Catalog Entities for Augment
# Generated on $(date)
#
...
metadata:
  name: augment-users
  namespace: default
  description: Users with access to Augment
  annotations:
    backstage.io/managed-by-location: "file:./augment-entities.yaml"
spec:
  type: team
  children: []
...
spec:
  ...
  memberOf: []
  # Note: user1 is NOT in augment-users, so will be denied access
EOF

    log_success "Generated: ${OUTPUT_DIR}/augment-entities.yaml"
}

# Generate Augment plugin configuration
generate_augment_config() {
    log_info "Generating Augment plugin configuration..."
    
    cat > "${OUTPUT_DIR}/app-config.augment.yaml" << EOF
# Augment Plugin Configuration
# Generated on $(date)
#
# Configure MCP servers that Augment connects to.
...
EOF
    ...
    log_success "Generated: ${OUTPUT_DIR}/app-config.augment.yaml"
}
...
# Complete Backstage Configuration for Augment
# Generated on $(date)
#
...
app:
  title: Developer Hub with Augment
...
print_summary() {
    ...
    echo "  - augment-entities.yaml: Catalog entities"
    echo "  - app-config.augment.yaml: Augment config"
    ...
    echo "               --config app-config.augment.yaml"
    echo ""
...
main() {
    echo "============================================"
    echo "  Augment - Backstage Config Generator"
    echo "============================================"
    echo ""
