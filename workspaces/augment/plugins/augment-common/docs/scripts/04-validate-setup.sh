#!/bin/bash
#
# Augment - Setup Validation Script
#
...
test_developer_token() {
    ...
    if [ -n "$DEVELOPER_TOKEN" ] && [ "$DEVELOPER_TOKEN" != "null" ]; then
        ...
        if [[ "$GROUPS" == *"augment-users"* ]]; then
            log_success "Token contains 'augment-users' group"
        else
            log_fail "Token missing 'augment-users' group. Found: ${GROUPS}"
        fi
...
# Test token generation for user1 (should NOT have augment-users group)
test_user1_token() {
    ...
    if [ -n "$USER1_TOKEN" ] && [ "$USER1_TOKEN" != "null" ]; then
        ...
        # Check groups - should NOT have augment-users
        GROUPS=$(echo "$CLAIMS" | jq -r '.groups // empty' 2>/dev/null || echo "")
        if [[ "$GROUPS" == *"augment-users"* ]]; then
            log_fail "User1 should NOT be in 'augment-users' group"
        else
            log_success "User1 correctly NOT in 'augment-users' group"
        fi
    else
...
# Test Augment health endpoint
test_augment_health() {
    log_section "Testing Augment Health"
    
    HTTP_CODE=$(curl -sk -o /tmp/te_health.json -w "%{http_code}" \
        "${BACKSTAGE_URL}/api/augment/health" 2>/dev/null || echo "000")
    
    if [ "$HTTP_CODE" == "200" ]; then
        STATUS=$(jq -r '.status // empty' /tmp/te_health.json)
        if [ "$STATUS" == "ok" ]; then
            log_success "Augment health: ok"
        else
            log_success "Augment health endpoint accessible"
        fi
    elif [ "$HTTP_CODE" == "000" ]; then
        log_skip "Augment not reachable"
    else
        log_fail "Augment health returned HTTP ${HTTP_CODE}"
    fi
}
...
test_developer_permission() {
    ...
    if [ "$RESULT" == "ALLOW" ]; then
        log_success "Developer has augment.access permission"
    elif [ "$RESULT" == "DENY" ]; then
        log_fail "Developer denied augment.access (check RBAC policies)"
    else
...
test_user1_permission() {
    ...
    if [ "$RESULT" == "DENY" ]; then
        log_success "User1 correctly denied augment.access"
...
# Test Augment API for developer
test_developer_api_access() {
    ...
    for endpoint in "workflows" "status" "prompt-groups"; do
        HTTP_CODE=$(curl -sk -o /dev/null -w "%{http_code}" \
            "${BACKSTAGE_URL}/api/augment/${endpoint}" \
            -H "Authorization: Bearer ${DEVELOPER_TOKEN}" 2>/dev/null || echo "000")
        
        if [ "$HTTP_CODE" == "200" ]; then
            log_success "Developer can access /api/augment/${endpoint}"
        elif [ "$HTTP_CODE" == "403" ]; then
            log_fail "Developer blocked from /api/augment/${endpoint}"
        elif [ "$HTTP_CODE" == "000" ]; then
            log_skip "Cannot reach /api/augment/${endpoint}"
        else
            log_info "Developer /api/augment/${endpoint} returned ${HTTP_CODE}"
        fi
    done
}

# Test Augment API for user1
test_user1_api_blocked() {
    ...
    for endpoint in "workflows" "status" "prompt-groups"; do
        ...
        if [ "$HTTP_CODE" == "403" ]; then
            log_success "User1 correctly blocked from /api/augment/${endpoint}"
        elif [ "$HTTP_CODE" == "200" ]; then
            log_fail "User1 should be blocked from /api/augment/${endpoint}"
        elif [ "$HTTP_CODE" == "000" ]; then
            log_skip "Cannot reach /api/augment/${endpoint}"
        else
            log_info "User1 /api/augment/${endpoint} returned ${HTTP_CODE}"
        fi
    done
}
...
print_summary() {
    ...
    if [ $TESTS_FAILED -eq 0 ]; then
        ...
        echo ""
        echo "Your Augment security setup is working correctly."
    else
