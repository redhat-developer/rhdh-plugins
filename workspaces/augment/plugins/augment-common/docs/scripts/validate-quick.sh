#!/bin/bash
#
# Quick validation script for Augment security setup
...
if [ -n "$KEYCLOAK_URL" ] && [ -n "$BACKSTAGE_CLIENT_SECRET" ]; then
    ...
    if [ -n "$TOKEN" ] && [ "$TOKEN" != "null" ]; then
        ...
        [ "$HAS_GROUP" == "true" ] && pass "Has augment-users group" || fail "Missing augment-users group"
...
if [ -n "$KEYCLOAK_URL" ] && [ -n "$BACKSTAGE_CLIENT_SECRET" ]; then
    ...
    if [ -n "$TOKEN" ] && [ "$TOKEN" != "null" ]; then
        ...
        if [ "$HAS_GROUP" != "true" ]; then
            pass "Correctly NOT in augment-users"
        else
            fail "Unexpectedly in augment-users"
        fi
...
# Test Augment health
HTTP=$(curl -sk -o /dev/null -w "%{http_code}" "${BACKSTAGE_URL}/api/augment/health" 2>/dev/null || echo "000")
[ "$HTTP" == "200" ] && pass "Augment health OK" || skip "Augment health returned $HTTP"
...
# Test Augment API health (no auth required for health)
section "Augment API"
HTTP=$(curl -sk -o /dev/null -w "%{http_code}" "${BACKSTAGE_URL}/api/augment/health" 2>/dev/null || echo "000")
if [ "$HTTP" == "200" ]; then
    pass "Augment API is running"
else
    fail "Augment API health returned $HTTP"
fi
