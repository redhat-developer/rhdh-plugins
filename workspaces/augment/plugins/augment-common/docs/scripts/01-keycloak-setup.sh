#!/bin/bash
#
# Augment - Keycloak Configuration Script
# 
# This script configures Keycloak for Augment authentication:
# - Creates/configures backstage client for OIDC authentication
# - Creates/configures mcp-server client for token validation
# - Creates augment-users group
...
setup_groups() {
    ...
    for GROUP_NAME in "augment-users" "rhdh-team"; do
...
# Setup test users
setup_users() {
    # Developer user - has access to Augment
    create_user "developer" "developer@example.com" "${DEVELOPER_PASSWORD}" "augment-users,rhdh-team"
    
    # User1 - does NOT have access to Augment
...
# Test Users
# developer:${DEVELOPER_PASSWORD} - Has augment.access (in augment-users group)
# user1:${USER1_PASSWORD} - Does NOT have augment.access
EOF
...
main() {
    echo "============================================"
    echo "  Augment - Keycloak Setup Script"
    echo "============================================"
    echo ""
