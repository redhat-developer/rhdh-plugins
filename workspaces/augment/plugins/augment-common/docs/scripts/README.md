# Augment Security Setup Scripts

Automated scripts to configure Keycloak and Backstage RBAC for Augment.

## ⚠️ Which Mode Are You Setting Up?

Augment supports **3 security modes**. Choose your mode first:
...
> **Note**: The MCP server scripts are included for reference but the `full` security mode (Augment → OAuth → MCP) is not yet fully tested. For now, use `plugin-only` mode where the MCP server uses its own service account for Kubernetes access.
...
| `augment-users` group        | Access control group              | `plugin-only`, `full` |
...
| `augment-entities.yaml`   | Catalog entities for users/groups |
| `app-config.augment.yaml` | Augment plugin config             |
...
yarn start --config app-config.yaml \
           ...
           --config app-config.augment.yaml
...
```
[FAIL] Token missing 'augment-users' group
```
...
```
[FAIL] Developer denied augment.access
```

**Solutions:**

1. Check RBAC policies are loaded
2. Verify group name format: `group:default/augment-users`
...
├── app-config.augment.yaml # Augment config
├── app-config.example.yaml      # Combined example
├── rbac-policy.csv              # RBAC policies
└── augment-entities.yaml  # Catalog entities
```
...
```
User → Keycloak → Backstage → Augment (OAuth token) → MCP Server
```

This requires:

- MCP server configured to validate Keycloak tokens
- Augment backend to fetch and forward OAuth tokens
...
- **Augment**: File issue in rhdh-plugins repository
