...
| --------- | ------------- | ------------- | --------------- |
| developer | developer123  | augment-users | ✅ Full Access  |
...
```json
{
  ...
  "groups": ["augment-users"]
}
```

**Verify**:

- [ ] `groups` contains `augment-users`
...
permission info permission.permission-evaluation
  meta={"userEntityRef":"user:default/user1","permissionName":"augment.access","result":"DENY"}
```

## Test 3: Augment Backend API

### 3.1 Test Health Endpoint (No Auth Required)

```bash
curl -s "${BACKSTAGE_URL}/api/augment/health" | jq .
```
...
curl -s "${BACKSTAGE_URL}/api/augment/workflows" \
...
curl -s -w "\nHTTP Status: %{http_code}\n" \
  "${BACKSTAGE_URL}/api/augment/workflows" \
...
```json
{
  "error": {
    ...
    "message": "You don't have permission to access Augment"
  }
}
HTTP Status: 403
```
...
for endpoint in workflows prompt-groups quick-actions status documents; do
  echo "Testing /api/augment/${endpoint}:"
...
4. Navigate to Augment

**Expected**:

- [ ] Login succeeds
- [ ] Augment shows "Access Denied" page
- [ ] Message: "You do not have the necessary permissions to access Augment"
...
2. Go to Network tab
3. Navigate to Augment
4. Check API calls

**Expected**:

- `/api/augment/workflows` returns 403
- `/api/augment/prompt-groups` returns 403
...
- [ ] developer can access Augment UI
...
2. Check group membership:
   ...
   ```bash
   curl -s "${BACKSTAGE_URL}/api/catalog/entities?filter=kind=group,metadata.name=augment-users" | jq '.items[0].spec.members'
   ```
...
echo "=== Augment Security Validation ==="
echo ""
...
DEV_PERM=$(curl -s -X POST "${BACKSTAGE_URL}/api/permission/authorize" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer ${DEVELOPER_TOKEN}" \
  -d '{"items":[{"permission":{"type":"basic","name":"augment.access","attributes":{"action":"read"}}}]}' \
  | jq -r '.items[0].result')
