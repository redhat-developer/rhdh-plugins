# Augment Security with Keycloak

This document describes the security architecture and configuration for the Augment plugin when integrated with Keycloak for authentication and authorization.
...
Augment supports **3 security modes** to accommodate different deployment scenarios:
...
Augment implements a multi-layer security model that integrates Keycloak with Backstage RBAC and MCP server security controls.
...
│  Groups: [group:default/augment-users, group:default/rhdh-team]       │
└──────────────────────────────────┬──────────────────────────────────────────┘
                                   │
                                   ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
│                                                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  Permission: augment.access                                    │    │
│  │                                                                      │    │
│  │  Policy Check:                                                       │    │
│  │    IF user IN group:default/augment-users                      │    │
│  │    THEN role:default/augment-user                              │    │
│  │    AND  role HAS permission augment.access → ALLOW             │    │
│  │                                                                      │    │
│  │  Enforcement Points:                                                 │    │
...
| Feature               | Status       | Description                                 |
| --------------------- | ------------ | ------------------------------------------- |
| Plugin Access Control | ✅ Supported | `augment.access` permission                 |
...
2. **Plugin Authorization** - Controls who can access Augment
