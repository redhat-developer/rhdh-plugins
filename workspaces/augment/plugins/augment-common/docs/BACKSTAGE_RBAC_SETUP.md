...
This guide explains how to configure Backstage RBAC to control access to the Augment plugin based on Keycloak groups.
...
3. Control access to Augment based on group membership
...
Augment uses a single permission:

| Permission       | Resource Type | Action | Description                          |
| ---------------- | ------------- | ------ | ------------------------------------ |
| `augment.access` | (none)        | read   | Controls access to the entire plugin |

This is an "all or nothing" permission:

- If ALLOW → User has full access to Augment
- If DENY → User cannot access any Augment features
...
```csv
# User to role
g, user:default/alice, role:default/augment-user
...
p, role:default/restricted-user, augment.access, read, deny
```
...
permission info permission.permission-evaluation
  meta={"userEntityRef":"user:default/developer","permissionName":"augment.access","result":"ALLOW"}
```
...
5. **Permission check requested** for `augment.access`
6. **RBAC policy evaluated**:
   - Is user in `group:default/augment-users`?
   - If yes → user has `role:default/augment-user`
   - Does role have `augment.access` permission?
   - If yes → ALLOW
   - Otherwise → DENY

## Frontend Behavior

The Augment frontend uses a `ConditionalPermissionWrapper` that respects the security mode:
...
Denied users see: "You do not have the necessary permissions to access Augment"

## Backend Behavior

All Augment API endpoints are protected:
...
- Response includes: `{"error":"You don't have permission to access Augment"}`
...
1. Verify augment-backend is loaded
