...
The Extensions CLI (`@red-hat-developer-hub/extensions-cli`) is a command-line tool for managing extensions plugin and package entities. It provides utilities to initialize, generate, verify, and export extensions entities.
...
- [`init`](#init) - Initialize extensions entities for a plugin
- [`generate`](#generate) - Generate Package entities from dynamic plugins configuration
- [`verify`](#verify) - Verify extensions entities
- [`export-csv`](#export-csv) - Export extensions entities to CSV files

---

## init

Interactive command to initialize extensions entities (Plugin and Package) for a new plugin.
...
The `init` command provides an interactive wizard to create extensions entities. It detects the plugin structure in the current directory and generates appropriate YAML entities.
...
Generate Package entities for the extensions catalog from a dynamic plugins configuration file.
...
The `generate` command reads a dynamic plugins configuration file and generates corresponding `Package` entities for the extensions catalog. This is useful for creating extensions entries from an existing RHDH deployment's plugin configuration.
...
Verify a set of extensions entities for consistency and completeness.
...
The `verify` command scans the current directory for YAML files and validates extensions `Plugin` and `Package` entities. It checks for:
...
```bash
cd /path/to/extensions-entities
npx @red-hat-developer-hub/extensions-cli verify
```
...
Export extensions plugin and package entities to CSV files for analysis or reporting.
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------ |
| `-p, --plugins-yaml-path [path]` | Path to the plugins folder containing extensions YAML files. Multiple paths can be separated by commas | **Required** |
...
The `export-csv` command parses extensions YAML files and generates CSV reports. This is useful for creating spreadsheets for plugin inventory, auditing, or documentation purposes.
...
```
reports/
├── extensions-plugins.csv
└── extensions-packages.csv
```
...
Represents a logical plugin in the extensions catalog. A plugin can contain one or more packages.
