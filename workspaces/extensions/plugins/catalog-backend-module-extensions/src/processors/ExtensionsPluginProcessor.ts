 ...
 * @public
 */
export class ExtensionsPluginProcessor implements CatalogProcessor {
  private readonly validators = [entityKindSchemaValidator(pluginJsonSchema)];

  getProcessorName(): string {
    return 'ExtensionsPluginProcessor';
  }
  ...
  async postProcessEntity(
    ...
    if (isExtensionsPlugin(entity)) {
      ...
      if (!entity.metadata.annotations?.[ExtensionsAnnotation.PRE_INSTALLED]) {
        entity.metadata.annotations = {
          ...entity.metadata.annotations,
          [ExtensionsAnnotation.PRE_INSTALLED]: 'false',
        };
      }

      // Align authors
      const authors: ExtensionsAuthor[] = [];
...
/**
 * @public
 * @deprecated Use ExtensionsPluginProcessor instead
 */
export const MarketplacePluginProcessor = ExtensionsPluginProcessor;
