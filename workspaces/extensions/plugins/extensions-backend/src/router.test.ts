...
type MockExtensionsEntity =
  | Partial<ExtensionsPlugin>
  | Partial<ExtensionsCollection>
  | Partial<ExtensionsPackage>;
    ...
    it.each(allTestCases)(
      ...
      async ({ description, reqBuilder, body, policyDecision }) => {
        ...
        const { backendServer } = await setupTestWithMockCatalog({
          mockData: [...mockPackages, ...mockPlugins],
          name,
          kind: isPackage ? ExtensionsKind.Package : ExtensionsKind.Plugin,
          config: FILE_INSTALL_CONFIG,
          policyDecision,
        });
