...
export const useCollections = (request: GetEntitiesRequest): AnyQueryResult => {
  const extensionsApi = useExtensionsApi();
  return useQuery({
    queryKey: ['extensionsApi', 'getCollections', request],
    queryFn: () => extensionsApi.getCollections(request),
