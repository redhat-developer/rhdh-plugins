...
const ExtensionsPackageContentSkeleton = () => {
  return (
    <Content>
...
const ExtensionsPackageContent = ({ pkg }: { pkg: ExtensionsPackage }) => {
...
export const ExtensionsPackageContentLoader = () => {
  const params = useRouteRefParams(packageInstallRouteRef);
  const pkg = usePackage(params.namespace, params.name);

  if (pkg.isLoading) {
    return <ExtensionsPackageContentSkeleton />;
  } else if (pkg.data) {
    return <ExtensionsPackageContent pkg={pkg.data} />;
  } else if (pkg.error) {
