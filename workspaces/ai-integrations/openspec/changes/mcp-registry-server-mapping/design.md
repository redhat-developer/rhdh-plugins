...
- Lossless capture of source data: every non-null scalar leaf is recoverable from a native field or a `modelcontextprotocol.io/*` annotation, except D9-redacted `isSecret` `default`/`value` leaves and D11-refused URL scalars (not copied into emitted URL fields and not projected as annotations).
...
**Choice:** A `server.json` that declares no `remotes` (e.g. a local-`packages`-only server), or whose remotes are all refused by the emitted-URL scheme policy (D11), is mapped to an entity with an **empty array** `spec.remotes: []`, never an omitted field.
