...
- Specify the **annotation projection** fallback: every `server.json` attribute that has no native home is projected into an annotation keyed `modelcontextprotocol.io/<dot.path.to.leaf>`, encoding nested objects and array indices as dot-separated segments within a single-slash, catalog-valid key. **Secret-flagged inputs** (`isSecret: true`) have their `default`/`value` leaves pruned so no credential is published into a searchable catalog annotation. **D11-refused URLs** are not projected.
...
- `mcp-registry-annotation-projection`: The fallback rule that projects every `server.json` attribute without a native home into `modelcontextprotocol.io/<dot.path>` annotations — dot-path encoding of nested objects and array indices, catalog-valid key sanitization/truncation, no-overwrite of natively-mapped fields, D11 URL allowlist on projected links, and scalar round-trip fidelity.
