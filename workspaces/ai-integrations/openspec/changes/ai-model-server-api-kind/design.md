...
1. **Field overrides** — When `rhdh.io/system`, `rhdh.io/serverType`, or `rhdh.io/default` annotations are present on the InferenceService, the kserve connector propagates them to `modelServer.annotations`. `ModelCatalogGenerator` reads these control annotations and maps them to spec-level fields (`spec.system`, `spec.serverType`, `spec.models.default` respectively), then deletes them from the annotation set so they do not appear in the emitted entity's `metadata.annotations`.
