...
import { isOciUrl } from './protocols';

/**
 * Extract the human-readable plugin name from an OCI package URL.
 *
 * Returns `null` for non-OCI formats.
 */
export function extractPluginName(pkg: string): string | null {
  if (!isOciUrl(pkg)) return null;
  return ociName(pkg);
}
