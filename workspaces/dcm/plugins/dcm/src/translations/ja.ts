...
> = createTranslationMessages({
  ...
  messages: {
    ...
    'deleteDialog.body': '{{resourceName}} を削除してもよろしいですか?この操作は元に戻せません。',
    'agents.emptyTitle': '登録されたエージェントはありません',
    'agents.emptyDescription': '環境エージェントはコントロールプレーンに登録され、定期的なハートビートを送信します。DCM が外部環境上のワークロードを管理できるようにするには、エージェントを登録します。',
    ...
    'agents.form.nameHelper': '固有のスラッグ識別子 ― 小文字、数字、ハイフンのみ',
    ...
    'agents.form.serviceTypesHelper': 'このエージェントが提供できるサービスタイプ',
    ...
    'agents.form.topicNameHelper': 'NATS トピック名 — dcm.agent で始まる必要があります。',
    'policies.emptyTitle': '定義されているポリシーはありません',
    'policies.emptyDescription': 'DCM リソースにガバナンスルールを適用するための OPA Rego ポリシーを作成します。ポリシーのスコープは、グローバルにまたはユーザーごとに指定できます。',
    ...
    'policies.form.policyTypeGlobal': 'GLOBAL — すべてのリクエストに適用されます',
    'policies.form.policyTypeUser': 'USER — ユーザーごとに適用されます',
    'policies.form.priorityLabel': '優先度 *',
    'policies.form.priorityHelper': '1 (最優先) – 1000 (最低)、デフォルトは 500 — ポリシータイプごとに一意である必要があります',
    'policies.form.regoCodeLabel': 'Rego コード *',
    'policies.form.regoCodeHelper': 'Placement Manager によって評価される OPA Rego ポリシー。',
    ...
    'serviceTypes.emptyDescription': 'サービスタイプは、カタログ項目のテンプレートスキーマを定義します。',
    ...
    'catalogItems.emptyDescription': 'カタログ項目とは、開発者がプロビジョニングできるサービステンプレートのことです。各カタログ項目は、1 つ以上のサービスタイプを参照し、カスタマイズ可能なフィールドを定義します。',
    ...
    'catalogItems.form.importTooltip': 'JSON または YAML カタログ項目の定義からフォームに入力します',
    'catalogItems.form.importError': 'ファイルのインポートに失敗しました — 有効な JSON または YAML 形式であることを確認してください。',
    'catalogItems.form.displayNameLabel': '表示名 *',
    'catalogItems.form.displayNameHelper': 'このカタログ項目の人間可読名 (最大 63 文字)',
    'catalogItems.form.apiVersionLabel': 'API バージョン *',
    'catalogItems.form.apiVersionHelper': 'パターン v<number>[alpha|beta][number] に従う必要があります — 例: v1、v1alpha1',
    'catalogItems.form.serviceTypeLabel': 'サービスタイプ *',
    'catalogItems.form.serviceTypeHelperEdit': 'サービスタイプは作成後に変更できません',
    'catalogItems.form.serviceTypeHelperNoTypes': '利用可能なサービスタイプがありません — サービスタイプタブで作成してください',
    'catalogItems.form.serviceTypeHelperDefault': 'このリソースの基となるサービスタイプを選択します',
    'catalogItems.form.fieldsLabel': 'フィールド *',
    'catalogItems.form.fieldsCaption': '(少なくとも 1 つ必須)',
    'catalogItems.form.fieldsErrorEmpty': 'パスが空でないフィールドを少なくとも 1 つ追加します。',
    'catalogItems.form.fieldAddButton': 'フィールドを追加する',
    'catalogItems.form.fieldAddTooltip': '新しいフィールドを追加する前に、直前のフィールドのパスを入力します',
    ...
    'catalogItems.form.fieldDefaultValueHelper': '任意の JSON 値 — 例: 42、"hello"、true、[1,2]',
    ...
    'catalogItems.form.schemaDialogHelper': 'JSON スキーマオブジェクト — 例: {"type":"integer","minimum":0}',
    'catalogItems.form.schemaDialogCancel': 'キャンセル',
    'catalogItems.form.schemaDialogApply': '適用',
    'catalogItems.form.schemaMustBeObject': '配列やプリミティブ型ではなく、JSON オブジェクトである必要があります',
    ...
    'catalogItems.wizard.resourcesDescription': '1 つ以上のリソースを追加します。各リソースはサービスタイプを参照し、独自のフィールド設定を定義します。',
    'catalogItems.wizard.resourcesRequired': '少なくとも 1 つのリソースが必要です。',
    ...
    'catalogItems.wizard.resourceNameHelper': 'このカタログ項目内の一意の識別子 — 例: app、ordersDb',
    'catalogItems.wizard.requiresResourcesLabel': 'リソースが必要',
    'catalogItems.wizard.requiresResourcesHelper': 'このリソースの前にプロビジョニングする必要のある他のリソースを選択します',
    'catalogItems.wizard.apiVersionImmutable': 'API バージョンは作成後に変更できません',
    'instances.emptyTitle': 'プロビジョニングされたインスタンスはありません',
    'instances.emptyDescription': 'カタログ項目インスタンスは、プロビジョニングされたサービスを表します。カタログ項目からインスタンスを作成し、登録済みの環境エージェント上でサービスをプロビジョニングします。',
    'instances.createButton': '作成',
    'instances.entityLabel': 'カタログ項目インスタンス',
    'instances.createDialogTitle': 'カタログ項目インスタンスを作成する',
    'instances.rehydrateSuccess': 'カタログ項目インスタンスが正常にリハイドレートされました。',
    ...
    'instances.rehydrateDialogBody': '{{instanceName}} をリハイドレートすると、リソースが再プロビジョニングされ、新しいリソース ID が割り当てられる場合があります。この操作は元に戻せません。',
    ...
    'instances.form.displayNameHelper': 'このプロビジョニングされたインスタンスの人間可読名 (最大 63 文字)',
    'instances.form.catalogItemLabel': 'カタログ項目 *',
    'instances.form.catalogItemSelect': 'カタログ項目を選択します…',
    'instances.form.catalogItemHelperNoItems': '利用可能なカタログ項目はありません — カタログ項目タブから作成してください',
    'instances.form.catalogItemHelperDefault': 'インスタンスをプロビジョニングするカタログ項目を選択します',
    'instances.form.apiVersionLabel': 'API バージョン *',
    'instances.form.apiVersionHelper': 'パターン v<number>[alpha|beta][number] に従う必要があります — 例: v1、v1alpha1',
    'instances.form.fieldValuesSection': 'フィールド値',
    'instances.form.fieldValuesSectionHint': '(このカタログ項目で定義された編集可能なフィールド)',
    'instances.form.noEditableFields': 'このリソースには編集可能なフィールドがありません。',
    'instances.wizard.tabOverview': '概要',
    'resources.emptyTitle': 'リソースが見つかりません',
    'resources.emptyDescription': 'DCM を通じてプロビジョニングされたサービスタイプのインスタンスはここに表示されます。',
    ...
    'validation.agent.namePattern': '使用できるのは小文字の英字、数字、ハイフンのみです (先頭は英字である必要があります)',
    'validation.agent.environmentRequired': '環境は必須です',
    'validation.agent.serviceTypesRequired': '少なくとも 1 つのサービスタイプが必要です',
    'validation.agent.costRequired': 'コストは必須です',
    'validation.agent.topicNameRequired': 'トピック名は必須です',
    'validation.agent.topicNamePattern': 'トピック名は dcm.agent で始まる必要があります。',
    ...
    'validation.policy.policyTypeOneOf': 'GLOBAL または USER である必要があります',
    ...
    'validation.policy.priorityMin': '優先度は少なくとも 1 である必要があります',
    'validation.policy.priorityMax': '優先度は最大 1000 までである必要があります',
    'validation.policy.regoCodeRequired': 'Rego コードは必須です',
    'validation.policy.regoCodeEmpty': 'Rego コードは空にできません',
    'validation.policy.regoCodePackage': 'パッケージ宣言を含める必要があります — 例: "package dcm.placement"',
    ...
    'validation.catalogItem.apiVersionPattern': 'パターン v<number>[alpha|beta][number] に従う必要があります — 例: v1、v1alpha1',
    'validation.catalogItem.serviceTypeRequired': 'サービスタイプは必須です',
    'validation.catalogItem.resourceNameRequired': 'リソース名は必須です',
    'validation.catalogItem.resourceNameDuplicate': 'リソース名はカタログ項目内で一意である必要があります',
    'validation.catalogItem.resourceNamePattern': '使用できるのは英字、数字、ハイフン、アンダースコアのみです (先頭は英字である必要があります)',
    'validation.catalogItem.requiresResourcesCycle': '循環依存関係が検出されました — このリソースは間接的に自身を要求しています',
    'validation.catalogItem.resourcesRequired': '少なくとも 1 つのリソースが必要です',
    'validation.catalogItem.duplicatePath': '重複するパス — パスは一意である必要があります',
    'validation.catalogItem.invalidJson': '無効な JSON — 構文を修正するか、プレーンな文字列値を使用します',
    'validation.catalogItem.schemaMustBeObject': 'JSON オブジェクトである必要があります — 例: {"type":"integer"}',
    'validation.catalogItem.schemaMinMaxConflict': '最小値 ({{min}}) は最大値 ({{max}}) を超えることはできません',
    'validation.catalogItem.defaultBelowMin': 'デフォルト値 ({{value}}) はスキーマの最小値 ({{min}}) を下回っています',
    'validation.catalogItem.defaultAboveMax': 'デフォルト値 ({{value}}) がスキーマの最大値 ({{max}}) を超えています',
    ...
    'validation.instance.apiVersionPattern': 'パターン v<number>[alpha|beta][number] に従う必要があります — 例: v1、v1alpha1',
