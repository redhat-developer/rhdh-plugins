...
export const useFilteredSupportTypes = () => {
  ...
  const items = useMemo(() => {
      ...
      const certifiedProviders = Array.from(
        new Set(
          certifiedPlugins
            .map(
              p =>
                p.metadata?.annotations?.[MarketplaceAnnotation.CERTIFIED_BY],
            )
            .filter(Boolean),
        ),
      ).join(', ');
    ...
    const customPlugins = availablePlugins.filter(
      plugin =>
        plugin.metadata?.annotations?.[MarketplaceAnnotation.PRE_INSTALLED] !==
        'true',
    );
    ...
    Object.entries(supportLevelCounts).forEach(([level, count]) => {
      if (level === MarketplaceSupportLevel.GENERALLY_AVAILABLE) {
        allSupportTypeItems.push({
      ...
      } else if (level === MarketplaceSupportLevel.TECH_PREVIEW) {
        allSupportTypeItems.push({
      ...
      } else if (level === MarketplaceSupportLevel.DEV_PREVIEW) {
        allSupportTypeItems.push({
      ...
      } else if (level === MarketplaceSupportLevel.COMMUNITY) {
        allSupportTypeItems.push({
