    ...
    >
      {data?.metrics?.map((metric: any) => (
        <Scorecard
          ...
          loading={false}
          statusColor={metric.result.thresholdResult.evaluation.status}
          StatusIcon={(() => {
            const label = metric.result.thresholdResult.evaluation.label;
            if (label === 'Ideal') return CheckCircleOutlineIcon;
