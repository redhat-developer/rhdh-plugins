...
describe('scorecard-backend-module-code-coverage', () => {
  let server: Server;
  ...
  it('starts the backend with the code-coverage module without errors', async () => {
    const res = await request(server).get('/api/scorecard/metrics');
    expect(res.status).toBe(200);
  });

  it('registers the code-coverage metric providers', async () => {
    const res = await request(server).get('/api/scorecard/metrics');
