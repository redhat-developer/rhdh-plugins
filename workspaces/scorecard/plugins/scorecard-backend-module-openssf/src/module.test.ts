...
describe('scorecard-backend-module-openssf', () => {
  let server: Server;
  ...
  it('starts the backend with the openssf module without errors', async () => {
    const res = await request(server).get('/api/scorecard/metrics');
    expect(res.status).toBe(200);
  });

  it('registers all 18 OpenSSF metric providers', async () => {
    const res = await request(server).get(
      '/api/scorecard/metrics?datasource=openssf',
    );
