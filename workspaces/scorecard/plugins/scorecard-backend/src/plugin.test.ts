...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [scorecardPlugin],
    });

    await request(server).get('/api/scorecard/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/scorecard/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/scorecard/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/scorecard/todos/${createdTodoItem.id}`)
