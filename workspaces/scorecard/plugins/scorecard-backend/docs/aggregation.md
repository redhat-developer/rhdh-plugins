...
The response shape includes **`id`**, **`status`**, **`metadata`** (title, description, type, aggregation type), and **`result`** (counts per threshold rule, total, thresholds). For **`average`**, **`result`** also includes **`averageScore`** (ratio in \[0, 1\]), **`averageWeightedSum`**, and **`averageMaxPossible`** (see backend README). The homepage card shows a donut gauge for this type instead of a multi-slice status pie.

Backend routing uses a small **aggregation strategy registry**: each KPI **`type`** maps to a class that loads data (often via shared status-count reads) and builds **`AggregatedMetricResult`**. New types register another strategy without extending `if` chains in the HTTP router.
...
For RBAC, scheduling, full endpoint reference, and **app-config examples** for **`average`** KPIs (including **`aggregationResultThresholds`**), see the [Scorecard backend README](../README.md).
