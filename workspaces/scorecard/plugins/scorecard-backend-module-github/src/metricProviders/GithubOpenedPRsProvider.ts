...
export class GithubOpenedPRsProvider implements MetricProvider<'number'> {
  ...
  getProviderId() {
    return 'OpenedPRsProvider';
  }

  getMetrics(): Metric<'number'>[] {
    return [
      {
        id: 'github.openedPRs7d',
        title: 'GitHub opened PRs (7d)',
        description:
          'Number of pull requests opened in the last 7 days for a given GitHub repository.',
        type: 'number',
  ...
  static fromConfig(config: Config): GithubOpenedPRsProvider {
    return new GithubOpenedPRsProvider(config);
  }
  ...
  async calculateMetrics(entity: Entity): Promise<Map<string, number>> {
    ...
    const result = await this.githubClient.getSearchCount(
      target,
      repository,
      `is:pr created:>${sinceStr}`,
    );

    const results = new Map<string, number>();
    results.set('github.openedPRs7d', result);
    return results;
  }
}
