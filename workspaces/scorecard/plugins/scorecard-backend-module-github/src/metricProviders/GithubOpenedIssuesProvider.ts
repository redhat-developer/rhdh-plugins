...
export class GithubOpenedIssuesProvider implements MetricProvider<'number'> {
  ...
  getProviderId() {
    return 'OpenedIssuesProvider';
  }

  getMetrics(): Metric<'number'>[] {
    return [
      {
        id: 'github.openedIssues7d',
        title: 'GitHub opened issues (7d)',
        description:
          'Number of issues opened in the last 7 days for a given GitHub repository.',
        type: 'number',
  ...
  static fromConfig(config: Config): GithubOpenedIssuesProvider {
    return new GithubOpenedIssuesProvider(config);
  }
  ...
  async calculateMetrics(entity: Entity): Promise<Map<string, number>> {
    ...
    const result = await this.githubClient.getSearchCount(
      target,
      repository,
      `is:issue created:>${sinceStr}`,
    );

    const results = new Map<string, number>();
    results.set('github.openedIssues7d', result);
    return results;
  }
}
