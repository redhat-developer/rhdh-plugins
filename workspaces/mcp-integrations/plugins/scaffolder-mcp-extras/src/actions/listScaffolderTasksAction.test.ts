...
/** Minimal response shape for listTasks API (replaces upstream openapi generated type) */
...
describe('createListScaffolderTasksAction', () => {
  ...
  it('should list tasks successfully', async () => {
    ...
    const expectedTasks = mockTasks.tasks.map(task => ({
      id: task.id,
      spec: task.spec,
      status: task.status,
  ...
  it('should pass limit and offset through to the API and return paginated results', async () => {
    ...
    const expectedTasks = paginatedResponse.tasks.map(task => ({
      id: task.id,
      spec: task.spec,
      status: task.status,
      createdAt: task.createdAt,
