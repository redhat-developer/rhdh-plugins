...
const lightspeedTranslationJa = createTranslationMessages({
  ...
  messages: {
    ...
    'lcore.loadError.description': 'Lightspeed バックエンドがモデル一覧を返しませんでした。サービスが実行中で到達可能か確認してから、もう一度お試しください。',
    'lcore.loadError.title': 'モデルを読み込めませんでした',
    'lcore.notConfigured.backendDocs': 'Lightspeed バックエンドのセットアップ',
    'lcore.notConfigured.description': 'Lightspeed を使用するには登録済みの LLM が必要です。セットアップを完了するには、組織のプラットフォーム管理者にお問い合わせください。',
    'lcore.notConfigured.developerLightspeedDocs': 'Developer Lightspeed を構成中',
    ...
    'notebook.overwrite.modal.description': '以下のファイルはこのノートブックに既に存在します。新しいバージョンで上書きしますか？',
    'notebook.overwrite.modal.title': 'ファイルを上書きしますか？',
    'notebook.upload.error.fileTooLarge': 'アップロードエラー: ファイルサイズが 25 MB の制限を超えています。',
    'notebook.upload.error.tooManyFiles': 'アップロードエラー: 最大 {{max}} ファイルまで許可されています。',
    'notebook.upload.error.unsupportedType': 'アップロードエラー: サポートされていないファイル形式が見つかりました。サポートされているファイル形式のみをアップロードしてください。',
    ...
    'notebook.view.documents.maxReached': '最大10個のドキュメントが許可されています。新しいドキュメントをアップロードするには、ドキュメントを削除してください。',
    'notebook.view.documents.uploading': 'ドキュメントをアップロード中',
    'notebook.view.input.disabledTooltip': 'チャットを開始するには、少なくとも1つのロード済みリソースを選択してください',
    ...
    'notebook.view.upload.heading': 'リソースをアップロードして開始してください',
