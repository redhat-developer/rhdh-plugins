...
describe('getCategorizeMessages', () => {
  ...
  it('categorizes messages into Favorites and Recent correctly', () => {
    const favoriteChats: string[] = ['1', '3'];
    const result = getCategorizeMessages(messages, favoriteChats, addProps);

    expect(result.Favorites).toHaveLength(2);
    expect(result.Favorites[0].text).toBe('First message');
    expect(result.Favorites[0].id).toBe('1');
    expect(result.Favorites[1].text).toBe('Third message');
    expect(result.Favorites[1].id).toBe('3');
  ...
  it('categorizes all messages as Recent when no favorites', () => {
    const favoriteChats: string[] = [];
    const result = getCategorizeMessages(messages, favoriteChats, addProps);
  ...
  it('categorizes all messages as Favorites when all are favorited', () => {
    const favoriteChats: string[] = ['1', '2', '3', '4', '5'];
    const result = getCategorizeMessages(messages, favoriteChats, addProps);

    expect(result.Favorites).toHaveLength(5);
    expect(result.Recent).toHaveLength(0);
    expect(result.Favorites[0].text).toBe('First message');
    expect(result.Favorites[4].text).toBe('Fifth message');
  });
  ...
  it('uses translation function when provided', () => {
    ...
    const mockT = jest.fn((key: string) => {
      if (key === 'conversation.category.favorites') return 'Favoritos';
