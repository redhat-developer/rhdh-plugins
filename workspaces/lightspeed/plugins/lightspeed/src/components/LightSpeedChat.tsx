...
export const LightspeedChat = ({
  ...
  const [isChatFavoritesEnabled, setIsChatFavoritesEnabled] = useState(true); // read from user settings in future
  const [favoriteChats, setFavoriteChats] = useState<string[]>([]); // read from user settings in future
  ...
  const addChatToFavorites = (convId: string) => {
    setFavoriteChats(prev => [...prev, convId]); // write to user settings in future
  };

  const removeChatFromFavorites = (convId: string) => {
    setFavoriteChats(prev => prev.filter(id => id !== convId)); // write to user settings in future
  };

  const additionalMessageProps = useCallback(
    (conversationSummary: ConversationSummary) => {
      const isChatFavorite = favoriteChats?.find(
            ...
            {isChatFavoritesEnabled && (
              <>
                {isChatFavorite ? (
                  ...
                  >
                    {t('conversation.removeFromFavorites')}
                  </DropdownItem>
                ) : (
                  <DropdownItem
                    onClick={() =>
                      addChatToFavorites(conversationSummary.conversation_id)
                    }
                  >
                    {t('conversation.addToFavorites')}
                  </DropdownItem>
                )}
              </>
            )}
  ...
  const categorizedMessages = useMemo(
    ...
    [additionalMessageProps, conversations, favoriteChats, t],
  );
  ...
  const filterConversations = useCallback(
    (targetValue: string) => {
      ...
      const filteredConversations = Object.entries(categorizedMessages).reduce(
        (acc, [key, items]) => {
          ...
          const isFavoritesCategory = key === favoritesKey;
          if (isFavoritesCategory && isChatFavoritesEnabled) {
            if (filteredItems.length > 0) {
              acc[favoritesKey] = filteredItems;
            } else {
              acc[favoritesKey] = [
                {
                  id: 'no-favorites',
                  text: t('chatbox.emptyState.noFavorites'),
                  noIcon: true,
                },
              ];
            }
    ...
    [categorizedMessages, isChatFavoritesEnabled, t],
  );
      ...
      >
        <ChatbotHeader className={classes.header}>
          ...
          <LightspeedChatBoxHeader
            ...
            models={models}
            isChatFavoritesEnabled={isChatFavoritesEnabled}
            onChatFavoritesToggle={setIsChatFavoritesEnabled}
          />
        </ChatbotHeader>
        <Divider />
